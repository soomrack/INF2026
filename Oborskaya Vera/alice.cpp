...
void print_detailed_report(int start_year, int start_month, int end_year, int end_month) {
    printf("\n\n");
    printf("========================================\n");
    printf("         ДЕТАЛЬНЫЙ ФИНАНСОВЫЙ ОТЧЁТ\n");
...
bool check_death(int month, int year) {
    ...
    if (death_chance < 2) { 
        ...
        if (who == 0) {
            printf(" %s  внезапно умер в возрасте %d лет\n",
                family.husband.name, family.husband.age);
            printf("   Месяц: %d, Год: %d\n", month, year);
        }
        else {
            printf(" %s  внезапно умерла в возрасте %d лет\n", family.wife.name, family.wife.age);
            printf("   Месяц: %d, Год: %d\n", month, year);
        }
...
void weather(Person* p, Person* q, int month, int year) {
    srand((unsigned)(time(NULL) + year * 12 + month));

    static int heatwave_months = 0;
    static int severe_winter_months = 0;
    static int flood_recovery_months = 0;
    static RUB storm_damage_recorded = 0;
    static int last_storm_year = 0;

    if (heatwave_months <= 0 && (month >= 6 && month <= 8)) {
        if (rand() % 100 < 15) {
            heatwave_months = 2 + (rand() % 3);
        }
    }

    if (heatwave_months > 0) {
        RUB extra_electricity = 2000 + (rand() % 5000);
        p->home.electricity += extra_electricity;
        q->home.electricity = p->home.electricity;
        stats.housing += extra_electricity * 2;
        stats.total_expenses += extra_electricity * 2;

        RUB extra_water = 800 + (rand() % 2000);
        p->home.water += extra_water;
        q->home.water = p->home.water;
        stats.housing += extra_water * 2;
        stats.total_expenses += extra_water * 2;

        RUB extra_food = 1500 + (rand() % 3000);
        p->food += extra_food;
        q->food += extra_food;
        stats.food += extra_food * 2;
        stats.total_expenses += extra_food * 2;

        heatwave_months--;
        if (heatwave_months == 0) {

            p->home.electricity -= extra_electricity;
            q->home.electricity -= extra_electricity;
            p->home.water -= extra_water;
            q->home.water -= extra_water;
            p->food -= extra_food;
            q->food -= extra_food;
        }
    }

    if (severe_winter_months <= 0 && (month == 12 || month == 1 || month == 2)) {
        if (rand() % 100 < 20) {
            severe_winter_months = 2 + (rand() % 2);
        }
    }

    if (severe_winter_months > 0) {
        RUB extra_heating = 3000 + (rand() % 7000);
        p->home.heating += extra_heating;
        q->home.heating = p->home.heating;
        stats.housing += extra_heating * 2;
        stats.total_expenses += extra_heating * 2;

        RUB extra_gas = 1000 + (rand() % 3000);
        p->home.gas += extra_gas;
        q->home.gas = p->home.gas;
        stats.housing += extra_gas * 2;
        stats.total_expenses += extra_gas * 2;

        RUB extra_car_cost = 2000 + (rand() % 5000);
        p->car.repair += extra_car_cost;
        q->car.repair += extra_car_cost;
        p->car.antifreeze += 1000;
        q->car.antifreeze += 1000;
        stats.car += extra_car_cost * 2 + 2000;
        stats.total_expenses += extra_car_cost * 2 + 2000;

        RUB extra_clothing = 3000 + (rand() % 5000);
        p->food += extra_clothing;
        q->food += extra_clothing;
        stats.food += extra_clothing * 2;
        stats.total_expenses += extra_clothing * 2;

        severe_winter_months--;
        if (severe_winter_months == 0) {
            p->home.heating -= extra_heating;
            q->home.heating -= extra_heating;
            p->home.gas -= extra_gas;
            q->home.gas -= extra_gas;
            p->car.repair -= extra_car_cost;
            q->car.repair -= extra_car_cost;
            p->car.antifreeze -= 1000;
            q->car.antifreeze -= 1000;
    ...
    if (flood_recovery_months <= 0 && (month == 3 || month == 4 || month == 7 || month == 8)) {
        if (rand() % 100 < 5) {
            flood_recovery_months = 2 + (rand() % 3);
            RUB flood_damage = 20000 + (rand() % 80000);
            RUB car_flood_damage = 5000 + (rand() % 30000);

            p->bank.account_rub -= flood_damage;
            q->bank.account_rub -= flood_damage / 2;
            stats.housing += flood_damage;
            stats.total_expenses += flood_damage;

            p->car.repair += car_flood_damage;
            q->car.repair += car_flood_damage / 2;
            stats.car += car_flood_damage;
            stats.total_expenses += car_flood_damage;

            p->car.value = (RUB)(p->car.value * 0.85f);
            q->car.value = (RUB)(q->car.value * 0.85f);
        }
    }


    if ((month == 9 || month == 10) && last_storm_year != year) {
        if (rand() % 100 < 8) {
            last_storm_year = year;
            RUB storm_damage = 10000 + (rand() % 40000);
            RUB car_damage = 3000 + (rand() % 20000);

            p->bank.account_rub -= storm_damage;
            q->bank.account_rub -= storm_damage / 2;
            stats.housing += storm_damage;
            stats.total_expenses += storm_damage;

            p->car.repair += car_damage;
            q->car.repair += car_damage;
            stats.car += car_damage;
            stats.total_expenses += car_damage;
        }
    }

    if (month >= 5 && month <= 8) {
        if (rand() % 100 < 6) {
            RUB hail_damage = 5000 + (rand() % 25000);


            p->car.repair += hail_damage;
            q->car.repair += hail_damage;
            stats.car += hail_damage;
            stats.total_expenses += hail_damage;
        }
    }

    static int drought_active = 0;
    if (!drought_active && (month == 5 || month == 6) && (rand() % 100 < 10)) {
        drought_active = 4;
    }

    if (drought_active > 0) {

        float food_increase = 1.15f + (rand() % 15) / 100.0f;
        RUB extra_food_cost_husband = (RUB)(p->food * (food_increase - 1.0f));
        RUB extra_food_cost_wife = (RUB)(q->food * (food_increase - 1.0f));

        p->food = (RUB)(p->food * food_increase);
        q->food = (RUB)(q->food * food_increase);
        stats.food += extra_food_cost_husband + extra_food_cost_wife;
        stats.total_expenses += extra_food_cost_husband + extra_food_cost_wife;

        drought_active--;


        if ((month == 4 || month == 5 || month == 9 || month == 10) && (rand() % 100 < 5)) {
            RUB frost_damage = 5000 + (rand() % 15000);

            p->bank.account_rub -= frost_damage;
            q->bank.account_rub -= frost_damage / 2;
            stats.housing += frost_damage;
            stats.total_expenses += frost_damage;


            RUB extra_heating = 2000 + (rand() % 5000);
            p->home.heating += extra_heating;
            q->home.heating += extra_heating;
            stats.housing += extra_heating * 2;
            stats.total_expenses += extra_heating * 2;
        ...
        if ((month == 6 || month == 7 || month == 8) && (rand() % 100 < 12)) {
            RUB health_cost = 1500 + (rand() % 4000);

            p->healthcare.regular_meds += health_cost;
            q->healthcare.regular_meds += health_cost;
            stats.healthcare += health_cost * 2;
            stats.total_expenses += health_cost * 2;
        }
    }

    static int dust_storm_active = 0;

    if (!dust_storm_active && (month >= 3 && month <= 8) && (rand() % 100 < 6)) {
        dust_storm_active = 1;
        RUB car_damage = 3000 + (rand() % 12000);
        RUB health_cost = 1500 + (rand() % 4000);
        ...
        stats.car += car_damage * 2;
        stats.healthcare += health_cost * 2;
        stats.total_expenses += car_damage * 2 + health_cost * 2;

        dust_storm_active = 0; 
    }

    static int wildfire_active = 0;
    static int wildfire_months_left = 0;

    if (!wildfire_active && (month >= 5 && month <= 9) && (rand() % 100 < 7)) {
        wildfire_active = 1;
        wildfire_months_left = 1 + (rand() % 2);
        RUB health_cost = 2500 + (rand() % 6000);
        RUB equipment_cost = 2000 + (rand() % 5000);


        p->healthcare.regular_meds += health_cost;
        q->healthcare.regular_meds += health_cost;
        p->home.furniture += equipment_cost; 

        stats.healthcare += health_cost * 2;
        stats.housing += equipment_cost;
        stats.total_expenses += health_cost * 2 + equipment_cost;
    }

    if (wildfire_active && wildfire_months_left > 0) {
        RUB ongoing_cost = 1000 + (rand() % 3000);
        p->healthcare.regular_meds += ongoing_cost;
        q->healthcare.regular_meds += ongoing_cost;
        stats.healthcare += ongoing_cost * 2;
        stats.total_expenses += ongoing_cost * 2;

        wildfire_months_left--;
        if (wildfire_months_left == 0) {
            wildfire_active = 0;
...
int main() {
    ...
    if (start_year < 2026 || end_year < start_year || start_year > 2080 ) {
        printf("Invalid period! Using default: 2026.1 - 2080.12\n");
        start_year = 2026;
        start_month = 1;
        end_year = 2028;
        end_month = 12;
