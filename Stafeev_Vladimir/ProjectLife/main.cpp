...
RUB  calculate_total_capital_kate();
RUB  calculate_total_capital_henry();

// --- Level 3: Low-level utilities ---
...
void update_computer_state(Computer& pc, const char* owner,
                           const char* device_name);
void init_business_computers(Business& business, int count);

// ============================================================================
// SECTION 6: LEVEL 3 — LOW-LEVEL UTILITIES
// (атомарные операции, не знают о других уровнях)
// ============================================================================
                            ...
                            const char* event_type)
{
    ...
    float rate = 0.0f;
    if      (strcmp(event_type, "accident") == 0) rate = policy.accident_coverage;
    else if (strcmp(event_type, "health")   == 0) rate = policy.health_coverage;
    else if (strcmp(event_type, "property") == 0) rate = policy.property_coverage;
    else if (strcmp(event_type, "business") == 0) rate = policy.business_coverage;
    else if (strcmp(event_type, "tech")     == 0) rate = policy.tech_coverage;

    RUB coverage = (RUB)(damage * rate);
...
void init_business_computers(Business& business, int count)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        business.office_computers[i].current_value =
            business.office_computers[i].purchase_price;
    }
...
// ============================================================================
// SECTION 7: LEVEL 2 — BUSINESS LOGIC
// (один сценарий на функцию, может вызывать L3-утилиты)
// ============================================================================
...
void init_character_kate()
{
    ...
    Kate.matiz.casco.annual_premium =
        calculate_insurance_premium(INSURANCE_EXTENDED, Kate.matiz.price,
                                    Kate.matiz.age, true);
    ...
    Kate.life_insurance = {INSURANCE_EXTENDED, 25000, 10000, 2000000,
                           0.9f, 0.8f, 0, 0, 0.5f, true, 0};
}
...
void init_character_henry()
{
    ...
    Henry.volga.casco.annual_premium =
        calculate_insurance_premium(INSURANCE_PREMIUM, Henry.volga.price,
                                    Henry.volga.age, true);
    ...
    Henry.life_insurance = {INSURANCE_PREMIUM, 40000, 5000, 5000000,
                            0.95f, 0.9f, 0, 0, 0.6f, true, 0};
}
...
void process_salary(Male& person, int month, int /*year*/)
{
    ...
    gross_salary += (RUB)(person.skills.managerial * 500);
...
void process_health_events(Female& person, int month, int /*year*/)
{
    ...
    if (random_event(sick_probability)) {
        ...
        RUB coverage      = process_insurance_claim(person.health.health_insurance,
                                                    sickness_cost, "health");
        RUB remaining     = sickness_cost - coverage;
    ...
    if (person.health.monthly_sport > 0 && random_event(0.02f)) {
        ...
        RUB coverage    = process_insurance_claim(person.health.health_insurance,
                                                  injury_cost, "health");
        RUB remaining   = injury_cost - coverage;
                        ...
                        const char* owner, InsurancePolicy& life_insurance)
{
    ...
    if (random_event(0.005f)) {
        ...
        RUB coverage      = process_insurance_claim(car.casco, accident_cost, "accident");
        printf("  %s: accident, cost: %lld RUB\n", owner, accident_cost);

        if (random_event(0.3f)) {
            RUB health_damage   = 10000 + rand() % 50000;
            RUB health_coverage = process_insurance_claim(life_insurance,
                                                          health_damage, "health");
            RUB total_coverage  = coverage + health_coverage;
            ...
            RUB remaining = accident_cost - coverage;
            if (remaining > 0) {
                if (emergency_fund >= remaining) {
                    emergency_fund -= remaining;
                } else {
                         ...
                         const char* owner)
{
    if (random_event(0.005f)) {
        ...
        RUB coverage    = process_insurance_claim(home.property_insurance,
                                                  repair_cost, "property");
        RUB remaining   = repair_cost - coverage;
    ...
    if (random_event(0.002f)) {
        ...
        RUB coverage   = process_insurance_claim(home.property_insurance,
                                                 theft_cost, "property");
        RUB remaining  = theft_cost - coverage;
...
void handle_computer_repair(Female& person, Computer& pc, const char* device_name)
{
    if (!pc.is_working && pc.purchase_price > 0) {
        ...
        RUB coverage    = process_insurance_claim(person.life_insurance,
                                                  repair_cost, "tech");
        RUB remaining   = repair_cost - coverage;
...
void process_deposits(Female& person, int month, int /*year*/)
{
    person.tbank.deposit += (RUB)(person.tbank.deposit *
                            (person.tbank.interest / 12.0f / 100.0f));
...
void process_deposits(Male& person, int month, int /*year*/)
{
    person.sber.deposit += (RUB)(person.sber.deposit *
                           (person.sber.interest / 12.0f / 100.0f));
    ...
    if (random_event(promotion_chance / 10.0f)) {
        ...
        switch (old_prof) {
            case PROFESSION_SPECIALIST:
                if (person.skills.technical  > 70.0f) new_prof = PROFESSION_SENIOR;    break;
            case PROFESSION_SENIOR:
                if (person.skills.managerial > 50.0f) new_prof = PROFESSION_TEAM_LEAD; break;
            case PROFESSION_TEAM_LEAD:
                if (person.skills.managerial > 70.0f) new_prof = PROFESSION_MANAGER;   break;
            case PROFESSION_MANAGER:
                if (person.skills.financial  > 60.0f) new_prof = PROFESSION_DIRECTOR;  break;
            default: break;
        }
...
void process_promotion(Male& person, int /*year*/)
{
    ...
    if (random_event(promotion_chance / 8.0f)) {
        ...
        switch (old_prof) {
            case PROFESSION_SENIOR:
                if (person.skills.managerial > 60.0f) new_prof = PROFESSION_TEAM_LEAD;    break;
            case PROFESSION_TEAM_LEAD:
                if (person.skills.managerial > 75.0f) new_prof = PROFESSION_MANAGER;      break;
            case PROFESSION_MANAGER:
                if (person.skills.financial  > 70.0f) new_prof = PROFESSION_DIRECTOR;     break;
            case PROFESSION_DIRECTOR:
                if (person.skills.financial  > 80.0f &&
                    person.emergency_fund    > 1000000)
...
void try_sell_business(Female& person, int /*year*/)
{
    ...
    RUB sell_price = (RUB)(person.business.current_value *
                           (0.8f + (rand() % 40) / 100.0f));
...
void calculate_business_tax(Female& person)
{
    RUB tax = (RUB)(person.business.monthly_profit * BUSINESS_TAX);
    if (tax < 0) tax = 0;

    person.business.tax_debt += tax;
    person.business.tax_paid += tax;
    person.business.account  -= tax;
}

void calculate_business_tax(Male& person)
{
    RUB tax = (RUB)((person.business.monthly_revenue -
...
void handle_business_risks(Female& person)
{
    if (random_event(0.02f)) {
        ...
        RUB coverage  = process_insurance_claim(person.business.business_insurance,
                                                tech_loss, "tech");
        tech_loss -= coverage;
...
void print_character_report_kate()
{
    ...
    printf("    Personal PC: %lld RUB (age: %d yr, performance: %.0f%%)\n",
           Kate.personal_pc.current_value, Kate.personal_pc.age,
           Kate.personal_pc.performance);
...
void print_character_report_henry()
{
    ...
    printf("    Gaming PC:   %lld RUB (age: %d yr, performance: %.0f%%)\n",
           Henry.gaming_pc.current_value, Henry.gaming_pc.age,
           Henry.gaming_pc.performance);
    printf("    Work laptop: %lld RUB (age: %d yr, performance: %.0f%%)\n",
           Henry.work_laptop.current_value, Henry.work_laptop.age,
           Henry.work_laptop.performance);
    printf("    Home server: %lld RUB (age: %d yr, performance: %.0f%%)\n",
           Henry.home_server.current_value, Henry.home_server.age,
           Henry.home_server.performance);
...
// Вынесено из print_final_report для соблюдения SRP:
// L2-функция считает капитал, L1-метод только выводит результат
RUB calculate_total_capital_kate()
{
    RUB tech_value = Kate.personal_pc.current_value
                   + Kate.laptop.current_value
                   + Kate.work_pc.current_value;

    RUB business_value = 0;
    if (Kate.business.is_active) {
        business_value = Kate.business.current_value + Kate.business.account;
        for (int i = 0; i < Kate.business.computer_count; i++)
            business_value += Kate.business.office_computers[i].current_value;
    }

    return Kate.tbank.account
         + Kate.tbank.deposit
         + Kate.flat.price
         + Kate.emergency_fund
...
RUB calculate_total_capital_henry()
{
    RUB tech_value = Henry.gaming_pc.current_value
                   + Henry.work_laptop.current_value
                   + Henry.home_server.current_value;

    RUB business_value = 0;
    if (Henry.business.is_active) {
        business_value = Henry.business.current_value + Henry.business.account;
        for (int i = 0; i < Henry.business.computer_count; i++)
            business_value += Henry.business.office_computers[i].current_value;
    }

    return Henry.sber.account
         + Henry.sber.deposit
         + Henry.house.price
...
// ============================================================================
// SECTION 8: LEVEL 1 — ORCHESTRATION
// (только вызовы L2-функций, никакой бизнес-логики)
// ============================================================================
...
void process_month(int month, int year)
{
    ...
    if (month == 1 || month == 6) {
        process_car_events(Kate.matiz,  Kate.tbank,  Kate.emergency_fund,
                           "Kate",  Kate.life_insurance);
        process_car_events(Henry.volga, Henry.sber,  Henry.emergency_fund,
...
void print_final_report()
{
    ...
    RUB kate_total  = calculate_total_capital_kate();
    RUB henry_total = calculate_total_capital_henry();
