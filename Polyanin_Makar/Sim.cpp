...
void alice_other_expenses(const int month, const int year) {
    ...
    // Одежда
    if (month == 4 || month == 9) {  
        alice.vtb.account_rub -= alice.clothes * 3;
    } else {
    ...
    // Транспорт
    alice.vtb.account_rub -= alice.transport;
    
    // Интернет
    alice.vtb.account_rub -= alice.internet;
    ...
    if (month == 12) {
        ...
        alice.vtb.account_rub -= gifts_cost;
    }
}
...
void bob_other_expenses(const int month, const int year) {
    ...
    // Развлечения
    bob.vtb.account_rub -= bob.entertainment;
    
    // Одежда
    if (month == 4 || month == 9) {
        bob.vtb.account_rub -= bob.clothes * 3;
    } else {
        bob.vtb.account_rub -= bob.clothes;
    }
    
    // Здоровье
    bob.vtb.account_rub -= bob.health;
    ...
    // Транспорт
    bob.vtb.account_rub -= bob.transport;

    // Интернет
    bob.vtb.account_rub -= bob.internet;
    ...
    if (month == 12) {
        ...
        bob.vtb.account_rub -= gifts_cost;
    }
}
...
void alice_manage_emergency_fund() {
    ...
    RUB monthly_expenses = alice.food + alice.utilities + alice.entertainment + 
                          ...
                          alice.internet + alice.cat.cfood + alice.car.gas;
...
void bob_manage_emergency_fund() {
    ...
    RUB monthly_expenses = bob.food + bob.utilities + bob.entertainment + 
                          ...
                          bob.internet + bob.cat.cfood + bob.car.gas;
    RUB target_emergency_fund = monthly_expenses * 6;
...
void alice_handle_health(const int month) {

    // Спортзал
    alice.vtb.account_rub -= alice.health_info.gym;

    // Лекарства
    alice.vtb.account_rub -= alice.health_info.medicines;

    // Платные врачи
    alice.vtb.account_rub -= alice.health_info.doctor;

    // Хронические болезни
    if (alice.health_info.has_chronic) {
        alice.vtb.account_rub -= alice.health_info.chronic_cost;
    }
}



void bob_handle_health(const int month) {

    // Спортзал
    bob.vtb.account_rub -= bob.health_info.gym;

    // Лекарства
    bob.vtb.account_rub -= bob.health_info.medicines;

    // Платные врачи
    bob.vtb.account_rub -= bob.health_info.doctor;

    // Хронические болезни
    if (bob.health_info.has_chronic) {
        bob.vtb.account_rub -= bob.health_info.chronic_cost;
    }
}
...
void alice_handle_pets(const int month) {

    if (alice.pets.has_dog) {
        alice.vtb.account_rub -= alice.pets.dog_food;
        alice.vtb.account_rub -= alice.pets.dog_vet;
        alice.vtb.account_rub -= alice.pets.dog_toys;
    }
...
void bob_handle_pets(const int month) {

    if (bob.pets.has_dog) {
        bob.vtb.account_rub -= bob.pets.dog_food;
        bob.vtb.account_rub -= bob.pets.dog_vet;
        bob.vtb.account_rub -= bob.pets.dog_toys;
    }
    bob_cat(month, 2026);
}
...
void simulation() {
    ...
    while (!(year == 2046 && month == 2)) { 
        ...
        alice_handle_health(month);
        bob_handle_health(month);
