...
public:
  bool on_fan = false;
  bool heater_was_on = false; //флаг что хиттер был включен
    ...
    if (fan_start_time > 0) {
      if (millis() - fan_start_time >= 20000)  {
        fan_start_time = 0; // 20 сек обдув
         heater_was_on = false; //сброс флага по окончании таймера
    }  else {
        on_fan = true;
    }
  }
}
//для запуска таймера после выключения хиттера
 void startHeaterCooldown() {
    if (!heater_was_on) {
      heater_was_on = true;
      fan_start_time = millis();
    }
...
void updateHeater() {
  if (pump.on_pump == true){
    //если хиттер выключается из-за помпы
    if (heater.on_heat) {
      fan.startHeaterCooldown(); //запускаем таймер охлаждения
    }
  ...
  if (air.temperature >= tomato.max_temperature) {
      //если хиттер был выключается из-за перегрева
      if (heater.on_heat) {
        fan.startHeaterCooldown(); //запускаем таймер охлаждения
      }
      heater.on_heat = false;
  ...
  //если температура в норме, но хиттер был включен
  if (air.temperature >= tomato.min_temperature && air.temperature < tomato.max_temperature) {
    if (heater.on_heat) {
      fan.startHeaterCooldown(); //запускаем таймер охлаждения
    }
    heater.on_heat = false;
...
void loop() {
  ...
  if (currentMillis - startTime < 20000) { //  приоритет на включение вентилятора при запуске
    fan.on_fan = true;
  } else {
    updateHeater();      
