...
static void init_person(person& p, const char* who) {
    // Zero all fields (avoids UB from uninitialized reads).
    memset(&p, 0, sizeof(p));
    p.age = 25;
...
static RUB estimate_monthly_income(const person& p, int month_index) {
    // Bonus in months 3, 6, 9, 12 (replaces shared static month_counter).
    RUB bonus = (month_index > 0 && month_index % 3 == 0) ? random_int(0, 12000) : 0;
...
static RUB estimate_monthly_income_after_tax(const person& p, int year_index) {
    // Fallback overload (assumes January). Main simulation passes real month.
    return estimate_monthly_income_after_tax(p, year_index, 1.0, 1);
}
    ...
    for (int month = 1; month <= 12; ++month) {
        ...
        if (DIARY_DAYS > 0) {
            for (int day = 0; day < 30; ++day) {
                // 12 months x 30 days per simulated year (not 365).
                int day_index = year_index * 360 + (month - 1) * 30 + day;
        ...
        // This makes "life by months" more continuous: repeated behavior nudges long-term traits.
        {
            ...
            // Work routine. More work days -> career growth and soft skills.
            if (cnt_work >= 14) {
                p.career_satisfaction = clamp_int(p.career_satisfaction + random_int(2, 5), 0, 100);
                p.soft_skills = clamp_int(p.soft_skills + random_int(1, 3), 0, 100);
                p.discipline_points = clamp_int(p.discipline_points + random_int(1, 3), 0, 100);
                p.stress_points = clamp_int(p.stress_points + random_int(0, 2), 0, 100);
            } else if (cnt_work >= 8) {
                p.career_satisfaction = clamp_int(p.career_satisfaction + random_int(1, 2), 0, 100);
                p.soft_skills = clamp_int(p.soft_skills + random_int(0, 1), 0, 100);
            }
