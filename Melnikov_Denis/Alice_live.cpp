...
{
    std::cout << "  RUB: " << account_rub << " руб.\n";
    if (account_usd > 0)
        std::cout << "  USD: " << account_usd
        << " (≈" << static_cast<RUB>(account_usd * dollar_rate()) << " руб.)\n";
    if (account_eur > 0)
        std::cout << "  EUR: " << account_eur
        << " (≈" << static_cast<RUB>(account_eur * euro_rate()) << " руб.)\n";
    std::cout << "  Итого в рублях: " << total_assets_rub() << " руб.\n";
}
...
// ── Новогодние: возрастные события + карьера + амортизация ──
// Выполняется в январе каждого года.
