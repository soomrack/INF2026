...
int Thermometer::min() const {
    return min_temperature;
}

int Thermometer::max() const {
    return max_temperature;
}
...
int Hygrometer::min() const {
    return min_RH;
}

int Hygrometer::max() const {
    return max_RH;
}
...
void control_temperature(const Thermometer& thermometer, Heater& heater, Fan& fan)
{
    if (thermometer.temperature >= thermometer.min()) {
        heater.on_heater = false;
        fan.on_fan = true;
    }
    else if (thermometer.temperature <= thermometer.max()) {
        heater.on_heater = true;
        fan.on_fan = false;
    }
    else {
...
void control_humidity_air(const Hygrometer& hygrometer, Pump& pump, Heater& heater, Fan& fan)
{
    if (hygrometer.RH <= hygrometer.min()) {
        pump.on_pump = true;
        heater.on_heater = true;
        fan.on_fan = true;
    }
    else if (hygrometer.RH >= hygrometer.max()) {
        pump.on_pump = false;
        heater.on_heater = false;
        fan.on_fan = true;
    }
...
void control_humidity_soil(const Hygrometer& hygrometer, Pump& pump)
{
    ...
    else if (hygrometer.RH >= hygrometer.min() + 10) {
        pump.on_pump = false;
    }
    else {
        pump.on_pump = false;
