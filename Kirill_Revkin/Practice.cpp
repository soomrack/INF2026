...
using Percent = int; // используем целый тип для процентов, т.к. логика износа и проверок (wear) рассчитана на целые значения;
...
struct Item {
	RUB value; 
	Percent wear;  // Износ в процентах (0 - новый, 100 - полностью изношен)        
	Percent monthly_wear;  // Ежемесячный износ в процентах
	...
	RUB service_cost;  // Ежемесячная стоимость обслуживания
};
...
void item_init(Item& it, const RUB value, const Percent wear, const Percent monthly_wear, const RUB repair_cost, const RUB replace_cost, const RUB service_cost) {  // Инициализация предмета с заданными параметрами
	it.value = value;
	it.wear = wear;
...
RUB person_total_card_money(const Person& person) {  // Суммарное количество денег на всех картах человека
...
void pay_from_all_cards(Person& person, const RUB amount, const char* what, const char* who) {  // Платеж с карт всех банков человека
	RUB rest = amount;
...
void person_cashback_tick(Person& person) {  // Начисление кэшбэка на карты человека
...
void pay_with_cards_and_credit(Person& person, Bank& prefer_card_bank, Bank& prefer_credit_bank, const RUB amount, const char* what, const char* who) {  // Платеж с использованием карт всех банков человека и кредита одного из банков человека
	if (amount <= 0) {
		return;
	}
...
void person_items_wear_tick(Person& person) {  // Увеличение износа всех предметов человека на ежемесячный износ
...
void try_repair_or_replace_item(Person& person, Item& it, const char* item_name, const char* who) {  // Попытка отремонтировать или заменить предмет, если его износ достиг определенных порогов
	if (it.value <= 0) {
		return;
	}
...
void david_salary(const int month, const int year) {  // Начисление зарплаты Дэвиду. Если месяц август 2026 года, то зарплата Дэвида увеличивается на 50% из-за повышения на работе
	if ((month == 8) && (year == 2026))
	{
...
void inflation_tick_person(Person& person) {  // Увеличение расходов человека на 10% в год из-за инфляции
...
void inflation_tick_home() {  // Увеличение расходов на содержание общего жилья на 10% в год из-за инфляции
	Percent inflation_month = (10 / 100.0 / 12.0);
...
void mortgage_interest_tick() {  // Начисление процентов по ипотеке на долг по ипотеке. Если долг по ипотеке равен нулю или меньше, то проценты не начисляются
	Percent mortgage_month = (SharedHome.mortgage_rate / 100.0 / 12.0);
	SharedHome.mortgage_debt += SharedHome.mortgage_debt * mortgage_month;
}


RUB person_salary_now(const Person& person) {  // Получение текущей зарплаты человека
...
RUB person_total_cash(const Person& person) {  // Получение общей суммы наличных денег человека, которая включает в себя деньги на картах всех банков человека и деньги в депозите банка, у которого самый высокий процент по депозиту
	return person.Tbank.card + person.Sberbank.card + person.Alfabank.card + person.VTB.card
		+ person.Tbank.deposit + person.Sberbank.deposit + person.Alfabank.deposit + person.VTB.deposit;
}


RUB person_total_credits(const Person& person) {   // Получение общей суммы долгов по кредитам человека, которая включает в себя долги по кредитам всех банков человека
	return person.Tbank.credit_debt + person.Sberbank.credit_debt + person.Alfabank.credit_debt + person.VTB.credit_debt;
}


RUB person_total_items(const Person& person) {  // Получение общей стоимости всех вещей человека, которая включает в себя стоимость всех категорий вещей человека
	RUB items = 0;
...
RUB person_total_assets(const Person& person) {  // Получение общей стоимости всех активов человека, которая включает в себя общую сумму наличных денег человека и общую стоимость всех вещей человека
...
RUB person_total_money_everywhere(const Person& person) {  // Получение общей суммы денег человека, которая включает в себя деньги на картах всех банков человека и деньги в депозитах всех банков человека
...
void goals_month_tick() {  // Месячный тик для целей, который включает в себя перевод денег с карты банка, у которого меньше всего денег на карте, на депозит банка, у которого самый высокий процент по депозиту для Алисы, для накопления на путешествия, ремонт и образование. Если после перевода на карте банка, у которого меньше всего денег на карте, останется меньше или равно определенной суммы для экстренных случаев, то перевод не выполняется, так как нужно оставить деньги на карте для экстренных случаев
...
void person_more_items_repair(Person& person, const char* who) {  // Попытка отремонтировать или заменить вещи человека
...
void person_more_items_service(Person& person, const char* who) {  // Оплата сервисов для вещей человека
	RUB cloud = 350;
	RUB antivirus = 220;
	RUB music = 279;
...
void all_month_tick(const int month, const int year) {  // Месячный тик для расширения, который включает в себя выполнение сезонного тика для расходов на еду и развлечения, выполнение тика для банков, выполнение сезонного тика для специальных расходов, выполнение тика для случайных событий, попытку улучшить салон татуировки, выполнение тика для целей и добавление нового лога за месяц в массив логов.
...
void print_person(const Person& person, const char* name) {  // Вывод информации о человеке
	printf("========== %s ==========\n", name);
