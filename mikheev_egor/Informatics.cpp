...
void food() {
	...
	RUB cost = random_coefficient % 800 + 400;
	if (Pasha.balance >= cost) {
		Pasha.balance_diff -= cost;
		return;
	}
	Pasha.health -= (1 - Pasha.balance / static_cast<float>(cost)) * 0.0001;
}
...
void random_shoping() {
	...
	RUB cost = random_coefficient % 5000;
	if (Pasha.balance >= cost) {
		Pasha.balance_diff -= cost;
		Pasha.health += 0.00001;
		return;
...
void new_year() {
	...
	RUB expense = (random_coefficient % 5000) * Pasha.friends;
...
void eight_march() {
	...
	RUB expense = (random_coefficient % 5000) * Pasha.friends / 2;
	if (Pasha.balance >= expense) {
		Pasha.balance_diff -= expense;
		return;
	}
...
void car_expenses() {
	...
	if (Pasha.car_grade == 1) current_car = &Granta;
	else if (Pasha.car_grade == 2) current_car = &Vesta;
	else if (Pasha.car_grade == 3) current_car = &Polo;
	else if (Pasha.car_grade == 4) current_car = &Superb;
	else if (Pasha.car_grade == 5) current_car = &m_911;
	else if (Pasha.car_grade == 6) current_car = &Urus;

	RUB total = current_car->gasoline_expense + current_car->repair_expense;
	if (Pasha.balance >= total) {
		Pasha.balance_diff -= current_car->gasoline_expense;
		Pasha.balance_diff -= current_car->repair_expense;
		Pasha.health += current_car->mental_satisfaction;
		return;
	}
	Pasha.car_grade -= 1;
...
void car_insurance() {
	...
	RUB insurance = 3000 + (Pasha.car_grade - 1) * 10000;
	if (Pasha.balance >= insurance) {
		Pasha.balance_diff -= insurance;
		return;
	}
	Pasha.balance_diff -= insurance;
	Pasha.health -= 0.00001;
...
void seasonal_clothing_update() {
	if (!((now_date.month == 3 && now_date.day == 15) ||
	        (now_date.month == 9 && now_date.day == 15))) {
		return;
	}
	RUB cost = 5000;
	if (Pasha.balance >= cost) {
...
void event_13_accident() {
	...
	if (Pasha.car_grade > 0) {
		...
		if (Pasha.balance >= 20000 * Pasha.car_grade) {
			...
			Pasha.car_grade = 0;
			Pasha.balance_diff += 500000 * Pasha.car_grade; // zero
		}
	} else {
...
void event_17_pet_illness() {
	...
	if (Pasha.event_decision != 1) {
		...
		if (Pasha.cat_life) Pasha.cat_life = 0;
		if (Pasha.dog_life) Pasha.dog_life = 0;
		Pasha.health -= 0.0002;
		std::cout << "--------------" << std::endl;
		return;
	}
