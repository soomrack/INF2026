...
void alice_apartment(int month, int year)
{
    ...
    alice.vtb.account -= alice.apartment.utilities * inflation_coef;

    // Ремонт (раз в 3 месяца)
    if (month % 3 == 0) {
        alice.vtb.account -= alice.apartment.repair * inflation_coef;
    }

    // Обновление мебели/техники (раз в год)
    if (month == 12) {
        alice.vtb.account -= 10000 * inflation_coef;  // мелкие покупки для дома
    }
...
void alice_health()  // здоровье
{
    alice.vtb.account -= alice.health.gym * inflation_coef;
    alice.vtb.account -= alice.health.doctor * inflation_coef;
    alice.vtb.account -= alice.health.pharmacy * inflation_coef;
}

void alice_monthly_expenses()
{
    alice.vtb.account -= alice.food * inflation_coef;
    alice.vtb.account -= alice.clothes * inflation_coef;
    alice.vtb.account -= alice.entertainment * inflation_coef;
    alice.vtb.account -= alice.education * inflation_coef;
    alice.vtb.account -= alice.gifts * inflation_coef;
    alice.vtb.account -= alice.internet * inflation_coef;
    alice.vtb.account -= alice.mobile * inflation_coef;
}

void alice_dog(int month, int year)
{
    alice.vtb.account -= alice.dog.food * inflation_coef;
    alice.vtb.account -= alice.dog.walking * inflation_coef;
    alice.vtb.account -= alice.dog.toys * inflation_coef;

    if (month % 2 == 0) {
        alice.vtb.account -= alice.dog.vet * inflation_coef;
    }

    if (month % 3 == 1) {
        alice.vtb.account -= alice.dog.grooming * inflation_coef;
    }

    if (month == 3) {
        alice.vtb.account -= alice.dog.insurance * inflation_coef;
    }

    if (month == 2 || month == 8) {
        alice.vtb.account -= alice.dog.training * inflation_coef;
    }
}
...
{
    if (month == 12) {
        alice.vtb.account -= 20'000 * inflation_coef;  // подарки
    }

    if (month == 9) {
        alice.vtb.account -= 15'000 * inflation_coef;  // празднование
    }

    if (month == 7 && year == 2027) {
        alice.vtb.account -= 80'000 * inflation_coef;  // отпуск
    }
}
...
void bob_business(int month, int year)
{
    ...
    if (profit > 0) {
        // Боб забирает 70% прибыли себе
        RUB bob_takes = profit * 70 / 100;

        // Добавляем на счет
        bob.vtb.account += bob_takes;

        // накапливаем общий доход от бизнеса
        bob.total_business_income += bob_takes;

        // 30% остается в бизнесе на развитие
...
{
    bob.vtb.account -= bob.dog.food * inflation_coef;
    bob.vtb.account -= bob.dog.walking * inflation_coef;
    bob.vtb.account -= bob.dog.toys * inflation_coef;

    if (month % 2 == 0) {
        bob.vtb.account -= bob.dog.vet * inflation_coef;
    }

    if (month % 3 == 1) {
        bob.vtb.account -= bob.dog.grooming * inflation_coef;
    }

    if (month == 4) {
        bob.vtb.account -= bob.dog.insurance * inflation_coef;
        bob.vtb.account -= bob.dog.training * inflation_coef;
    }
}
...
{
    bob.vtb.account -= bob.luxury * inflation_coef;

    if (month % 2 == 0) {  // рестораны
        bob.vtb.account -= 30'000 * inflation_coef;
    }

    if (month == 8 && year == 2026) {  // Путешествия
        bob.vtb.account -= 200'000 * inflation_coef;
    }

    if (month >= 6 && month <= 8 && year == 2027) {  // Яхта (летом)
        bob.vtb.account -= 150'000 * inflation_coef;
    }
}
...
void bob_health()
{
    bob.vtb.account -= bob.health.gym * inflation_coef;
    bob.vtb.account -= bob.health.doctor * inflation_coef;
    bob.vtb.account -= bob.health.dentist * inflation_coef;
    bob.vtb.account -= bob.health.pharmacy * inflation_coef;
    bob.vtb.account -= bob.health.cosmetics * inflation_coef;
}
...
{
    // Генерируем меню на текущий месяц
    bob_generate_monthly_menu(month, year);

    // Тратим на продукты по меню
    bob.vtb.account -= bob.nutrition.current_menu.total_cost * inflation_coef;

    // Тратим на спортпит
    bob.vtb.account -= bob.nutrition.protein_shakes * inflation_coef;
    bob.vtb.account -= bob.nutrition.vitamins * inflation_coef;
    bob.vtb.account -= bob.nutrition.sports_supplements * inflation_coef;

    // Доставка готовой еды (если есть)
    if (month % 2 == 0) {  // раз в 2 месяца заказывает доставку
        bob.vtb.account -= bob.nutrition.meal_delivery * inflation_coef;
    }

    // Рестораны (в выходные)
    if (month == 6 || month == 12) {  // в июне и декабре чаще ходит в рестораны
        bob.vtb.account -= bob.nutrition.restaurant_visits * 2 * inflation_coef;
    }
    else {
        bob.vtb.account -= bob.nutrition.restaurant_visits * inflation_coef;
    }

    // Проверяем бюджет
    RUB total_nutrition = bob.nutrition.current_menu.total_cost +
    ...
    if (total_nutrition > bob.nutrition.monthly_budget) {
        printf("  Боб: превышение бюджета на питание на %lld руб\n",
            total_nutrition - bob.nutrition.monthly_budget);
    }
}
...
{
    if (month == 7 && year == 2027) {
        bob.vtb.account -= bob.vacation.flights * inflation_coef;
        bob.vtb.account -= bob.vacation.hotel * inflation_coef;
        bob.vtb.account -= bob.vacation.food * inflation_coef;
        bob.vtb.account -= bob.vacation.entertainment * inflation_coef;
        bob.vtb.account -= bob.vacation.shopping * inflation_coef;
        bob.vtb.account -= bob.vacation.transport * inflation_coef;
    }
}
...
void bob_cat()
{
    bob.vtb.account -= bob.cat.food * inflation_coef;
    bob.vtb.account -= bob.cat.grooming * inflation_coef;
    bob.vtb.account -= bob.cat.vet * inflation_coef;
    bob.vtb.account -= bob.cat.toys * inflation_coef;
}
...
// 1. БАЗОВЫЕ СОБЫТИЯ
void bob_base_events(int month, int year)
{
...
// 3. БИЗНЕС-УСПЕХИ (доходы бизнеса не умножаем на инфляцию)
...
// 7. ЛЮКС ТРАТЫ (НЕЗАПЛАНИРОВАННЫЕ)
...
// 8. ПРОБЛЕМЫ СО ЗДОРОВЬЕМ (ЭЛИТНАЯ МЕДИЦИНА)
...
void bob_misc_events(int month, int year)
{
    ...
    if (month == 4 && year == 2027) {
        bob.luxury += 50'000 * inflation_coef;  // ИНФЛЯЦИЯ
...
{
    // Ежемесячные расходы
    alice.vtb.account -= alice.car.maintenance * inflation_coef;
    alice.vtb.account -= alice.car.wash * inflation_coef;
    alice.vtb.account -= alice.car.parking * inflation_coef;
    alice.vtb.account -= alice.car.gas * inflation_coef;

    // Сезонные расходы
    if (month == 11) {
        alice.vtb.account -= alice.car.tires * inflation_coef;
    }

    if (month == 1) {
        alice.vtb.account -= alice.car.insurance * inflation_coef;
    }

    if (month == 8) {
        alice.vtb.account -= alice.car.tax * inflation_coef;
    }
}
...
void alice_cat()
{
    alice.vtb.account -= alice.cat.food * inflation_coef;
    alice.vtb.account -= alice.cat.vet * inflation_coef;
    alice.vtb.account -= alice.cat.grooming * inflation_coef;
    alice.vtb.account -= alice.cat.toys * inflation_coef;
}
...
void alice_random_events(int month, int year)
{
...
void simulation()
{
    ...
    while (!(month == 3 and year == 2028)) {
        ...
        if (month == 13) {
            ...
            inflation_coef *= (1.0 + inflation_rate);  // увеличение коэффициента
