...
{
    if (rick.bmw_x3.credit <= 0) return;
...
void rick_bmw_x3_repair(const int month, const int year)
{
    ...
    if (random_chance >= repair_chance) return;
...
{
    if (rick.bmw_x5.credit <= 0) return;
...
void rick_housing_repairs(int month, int year)
{
    ...
    if (event_chance >= threshold) return;
        int housing_event;
        RUB event_cost = 0;
...
{
    if (month == 12 and rick.has_house) {
