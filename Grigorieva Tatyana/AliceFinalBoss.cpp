...
struct Pet {
    int has_pet;                // есть ли питомец (0 или 1)
    enum PetType type;          // тип питомца
    char name[30];              // имя питомца
    int age_months;             // возраст в месяцах
    RUB monthly_food_cost;      // расходы на корм в месяц
    RUB monthly_vet_cost;       // расходы на ветеринара в месяц
    RUB monthly_toys_cost;      // расходы на игрушки
    int happiness;              // счастье питомца (0-100)
    int health;                 // здоровье питомца (0-100)
    int last_vet_visit;         // месяц последнего визита к ветеринару
};
...
struct Achievement achievements[] = {
    {" Миллионер", "Накопить 1 000 000 рублей", 0},
    {" Домовладелица", "Купить квартиру (погасить ипотеку)", 0},
    {" Любитель животных", "Завести питомца", 0},
    {" Трудоголик", "Получить зарплату выше 400 000 рублей", 0},
    {" Здоровье превыше всего", "Потратить на лечение больше 100 000 рублей", 0},
    {" Главное - не уработаться", "Сходить в отпуск", 0},
    {" Везунчик", "Получить наследство", 0},
    {" Старожил", "Прожить в симуляции больше 25 лет", 0},
    {" Вкладчик", "Иметь на сбережениях больше 500 000 рублей", 0}
};

int achievements_count = sizeof(achievements) / sizeof(achievements[0]);

int total_simulation_months = 0;
...
void alice_simulation(int years);
void print_results();

void check_achievements(struct Person* p, int year, int month);
...
// Прототипы функций для питомца
void init_pet(struct Person* p);
void pet_happiness_change(struct Person* p);
void pet_health_change(struct Person* p);
RUB calculate_pet_monthly_cost(struct Person* p);
void process_pet(struct Person* p, int month);
void pet_event(struct Person* p);
void print_pet_info();
...
void alice_init()
{
    ...
    init_pet(&alice);      // заводим питомца
...
RUB cook_meal(struct Fridge &fridge, const Recipe &recipe, RUB* total_cost) {
    ...
    for (int i = 0; i < recipe.num_ingredients; i++) {  // ← точка, а не стрелка
...
void update_product_prices(struct Fridge &fridge, int month) {
    ...
    for (int i = 0; i < fridge.num_products; i++) {
        ...
        fridge.products[i].price_per_unit = 
            fridge.products[i].base_price * pow(multiplier, month - 1);  // ← точка вместо ->
    }
}


// ============================================================
//  РЕАЛИЗАЦИЯ ФУНКЦИЙ ДЛЯ ПИТОМЦА
// ============================================================
void init_pet(struct Person* p) {
    // Решаем, заводить ли питомца (30% шанс)
    if (rand() % 100 < 30) {
        p->pet.has_pet = 1;
        
        // Случайно выбираем тип питомца
        int type_index = rand() % 5;
        p->pet.type = (enum PetType)type_index;
        strcpy(p->pet.name, pet_type_names[type_index]);
        
        // Генерируем случайное имя
        const char* dog_names[] = {"Бобик", "Шарик", "Рекс", "Джек", "Лайм"};
        const char* cat_names[] = {"Борис", "Барсик", "Рыжик", "Снежок", "Васька"};
        const char* bird_names[] = {"Кеша", "Гоша", "Петруша", "Рики"};
        const char* hamster_names[] = {"Хома", "Шустрик", "Пушистик"};
        const char* fish_names[] = {"Немо", "Дори", "Кит"};
        
        switch(p->pet.type) {
            case PET_DOG:
                strcpy(p->pet.name, dog_names[rand() % 5]);
                p->pet.monthly_food_cost = 3000 + rand() % 2000;
                p->pet.monthly_vet_cost = 500 + rand() % 1000;
                p->pet.monthly_toys_cost = 500 + rand() % 1000;
                break;
            case PET_CAT:
                strcpy(p->pet.name, cat_names[rand() % 5]);
                p->pet.monthly_food_cost = 2000 + rand() % 1500;
                p->pet.monthly_vet_cost = 400 + rand() % 800;
                p->pet.monthly_toys_cost = 300 + rand() % 700;
                break;
            case PET_PARROT:
                strcpy(p->pet.name, bird_names[rand() % 4]);
                p->pet.monthly_food_cost = 800 + rand() % 700;
                p->pet.monthly_vet_cost = 300 + rand() % 500;
                p->pet.monthly_toys_cost = 200 + rand() % 500;
                break;
            case PET_HAMSTER:
                strcpy(p->pet.name, hamster_names[rand() % 3]);
                p->pet.monthly_food_cost = 500 + rand() % 500;
                p->pet.monthly_vet_cost = 200 + rand() % 400;
                p->pet.monthly_toys_cost = 100 + rand() % 300;
                break;
            case PET_FISH:
                strcpy(p->pet.name, fish_names[rand() % 3]);
                p->pet.monthly_food_cost = 300 + rand() % 300;
                p->pet.monthly_vet_cost = 100 + rand() % 200;
                p->pet.monthly_toys_cost = 50 + rand() % 150;
                break;
        }
        
        p->pet.age_months = rand() % 24;  // от 0 до 24 месяцев
        p->pet.happiness = 70 + rand() % 30;  // 70-100
        p->pet.health = 80 + rand() % 20;     // 80-100
        p->pet.last_vet_visit = 0;
        
        printf("\n У АЛИСЫ ПОЯВИЛСЯ ПИТОМЕЦ!\n");
        printf("  Тип: %s\n", pet_type_names[p->pet.type]);
        printf("  Имя: %s\n", p->pet.name);
        printf("  Возраст: %d мес.\n", p->pet.age_months);
        printf("  Расходы в месяц: %lld руб.\n", 
               p->pet.monthly_food_cost + p->pet.monthly_vet_cost + p->pet.monthly_toys_cost);
    } else {
        p->pet.has_pet = 0;
        printf("\n У Алисы пока нет питомца\n");
    }
}

void pet_happiness_change(struct Person* p) {
    if (!p->pet.has_pet) return;
    
    int change = 0;
    
    // Случайные события, влияющие на счастье питомца
    int random_event = rand() % 100;
    
    if (random_event < 15) {
        // Хорошее событие
        change = 5 + rand() % 15;
        printf("   %s очень рад(а)! (+%d счастья)\n", p->pet.name, change);
    } else if (random_event < 30) {
        // Плохое событие
        change = -(5 + rand() % 15);
        printf("   %s грустит... (%d счастья)\n", p->pet.name, change);
    } else {
        // Нейтрально
        change = -2 + rand() % 5;
    }
    
    p->pet.happiness += change;
    
    // Ограничиваем счастье от 0 до 100
    if (p->pet.happiness > 100) p->pet.happiness = 100;
    if (p->pet.happiness < 0) p->pet.happiness = 0;
}
...
void pet_health_change(struct Person* p) {
    ...
    if (p->pet.happiness < 30) {
        change -= 5;
    } else if (p->pet.happiness > 70) {
        change += 3;
    }
    
    // Возраст влияет на здоровье (чем старше, тем хуже)
    if (p->pet.age_months > 60) {  // старше 5 лет
        change -= 4;
    } else if (p->pet.age_months > 36) {  // старше 3 лет
        change -= 2;
    }
    ...
    if (p->pet.health > 100) p->pet.health = 100;
    if (p->pet.health < 0) p->pet.health = 0;
    
    if (change != 0) {
        printf(" Здоровье %s: %d → %d\n", p->pet.name, 
               p->pet.health - change, p->pet.health);
    }
}

RUB calculate_pet_monthly_cost(struct Person* p) {
    if (!p->pet.has_pet) return 0;
    
    RUB total = p->pet.monthly_food_cost + p->pet.monthly_toys_cost;
    
    // Нужен ли визит к ветеринару? (раз в 6 месяцев)
    if (p->pet.last_vet_visit == 0 || p->pet.last_vet_visit >= 6) {
        total += p->pet.monthly_vet_cost;
        p->pet.last_vet_visit = 0;
        printf("  🏥 Визит к ветеринару: %lld руб.\n", p->pet.monthly_vet_cost);
        
        // Визит к ветеринару улучшает здоровье
        p->pet.health += 10;
        if (p->pet.health > 100) p->pet.health = 100;
    } else {
        p->pet.last_vet_visit++;
    }
...
void pet_event(struct Person* p) {
    ...
    // Редкие события с питомцем (2% в месяц)
    if (rand() % 100 < 2) {
        int event_type = rand() % 4;
        
        switch(event_type) {
            case 0:
                printf("\n  СОБЫТИЕ: %s принёс найденную вещь!\n", p->pet.name);
                p->bank.card += 1000;
                stats.salary_received += 1000;
                p->pet.happiness += 10;
                break;
            case 1:
                printf("\n  СОБЫТИЕ: %s что-то сломал в квартире!\n", p->pet.name);
                p->bank.card -= 2000;
                stats.total_events_impact += 2000;
                p->pet.happiness -= 10;
                break;
            case 2:
                printf("\n  СОБЫТИЕ: %s выиграл конкурс питомцев!\n", p->pet.name);
                p->bank.card += 5000;
                stats.salary_received += 5000;
                p->pet.happiness += 20;
                break;
            case 3:
                printf("\n  СОБЫТИЕ: %s заболел! Нужен срочный ветврач\n", p->pet.name);
                RUB cost = 3000 + rand() % 10000;
                p->bank.card -= cost;
                stats.total_events_impact += cost;
                p->pet.health -= 20;
                printf("    Лечение обошлось в %lld руб.\n", cost);
...
void process_pet(struct Person* p, int month) {
    ...
    printf("\n ПИТОМЕЦ: %s (%s)\n", p->pet.name, pet_type_names[p->pet.type]);
    printf("  Возраст: %d мес.\n", p->pet.age_months);
    printf("  Счастье: %d/100\n", p->pet.happiness);
    printf("  Здоровье: %d/100\n", p->pet.health);
    
    // Обновляем возраст питомца каждый месяц
    p->pet.age_months++;
    
    // Меняем счастье и здоровье
    pet_happiness_change(p);
    pet_health_change(p);
    
    // Рассчитываем расходы на питомца
    RUB pet_cost = calculate_pet_monthly_cost(p);
    
    if (pet_cost > 0) {
        printf("  Расходы на питомца: %lld руб.\n", pet_cost);
        
        // Оплачиваем расходы (50% наличными, 50% картой)
        RUB pet_cash = pet_cost * 0.5;
        RUB pet_card = pet_cost * 0.5;
        
        if (p->cash >= pet_cash) {
            p->cash -= pet_cash;
        } else {
            RUB needed = pet_cash - p->cash;
            p->cash = 0;
            p->bank.card -= needed;
            pet_card += needed;
            stats.cash_withdrawn += needed;
        }
        
        p->bank.card -= pet_card;
        
        stats.cash_spent += pet_cash;
        stats.card_spent += pet_card;
        
        // Кэшбэк на расходы для питомца
        if (p->main_bank != NULL) {
            RUB cashback = pet_card * p->main_bank->card_cashback;
            p->bank.card += cashback;
            stats.cashback_received += cashback;
            if (cashback > 0) printf("  Кэшбэк за питомца: %lld руб.\n", cashback);
        }
    } else {
        printf("  Расходов на питомца нет\n");
    }
    
    // Проверяем здоровье питомца
    if (p->pet.health <= 20) {
        printf("\n  ВНИМАНИЕ! %s очень болен! Нужно срочно к ветеринару!\n", p->pet.name);
    }
    
    if (p->pet.health <= 0) {
        printf("\n  ГРУСТНОЕ СОБЫТИЕ: %s скончался(лась)...\n", p->pet.name);
        p->pet.has_pet = 0;
    }
...
void print_pet_info() {
    if (!alice.pet.has_pet) {
        printf("  Питомец: нет\n");
        return;
    }
    
    printf("Питомец: %s (%s)\n", alice.pet.name, pet_type_names[alice.pet.type]);
    printf("Возраст: %d мес.\n", alice.pet.age_months);
    printf("Счастье: %d/100\n", alice.pet.happiness);
    printf("Здоровье: %d/100\n", alice.pet.health);
    
    // Совет по уходу
    if (alice.pet.happiness < 30) {
        printf("Совет: поиграйте с питомцем!\n");
    }
    if (alice.pet.health < 40) {
        printf("Совет: посетите ветеринара!\n");
    }
}

// Ачивки
void check_achievements(struct Person* p, int year, int month) {
    RUB total_wealth = p->cash + p->bank.card + p->bank.savings + p->car.value;
    static int years_passed = 0;
    
    years_passed = year - 2026;
    
    // Миллионер
    if (!achievements[0].achieved && total_wealth >= 1000000) {
        achievements[0].achieved = 1;
        printf("\n ДОСТИЖЕНИЕ: %s", achievements[0].name);
        printf("\n   %s\n", achievements[0].description);
    }
    
    // Домовладелица (ипотека выплачена и нет аренды)
    if (!achievements[1].achieved && !p->mortgage.active && p->housing.rent == 0) {
        achievements[1].achieved = 1;
        printf("\n ДОСТИЖЕНИЕ: %s", achievements[1].name);
        printf("\n   %s\n", achievements[1].description);
    }
    
    // Любитель животных (есть питомец)
    if (!achievements[2].achieved && p->pet.has_pet) {
        achievements[2].achieved = 1;
        printf("\n ДОСТИЖЕНИЕ: %s", achievements[2].name);
        printf("\n   %s\n", achievements[2].description);
    }
    
    // Трудоголик (зарплата выше 400 000)
    if (!achievements[3].achieved && p->salary >= 400000) {
        achievements[3].achieved = 1;
        printf("\n ДОСТИЖЕНИЕ: %s", achievements[3].name);
        printf("\n   %s\n", achievements[3].description);
    }
    
    // Здоровье превыше всего (лечение больше 100 000)
    if (!achievements[4].achieved && stats.tax_deduction_base >= 100000) {
        achievements[4].achieved = 1;
        printf("\n ДОСТИЖЕНИЕ: %s", achievements[4].name);
        printf("\n   %s\n", achievements[4].description);
    }
    
    // Главное - не уработаться (был в отпуске)
    for (size_t i = 0; i < events.size(); i++) {
        if (events[i].triggered && strstr(events[i].description, "Отпуск") != NULL) {
            if (!achievements[5].achieved) {
                achievements[5].achieved = 1;
                printf("\n ДОСТИЖЕНИЕ: %s", achievements[5].name);
    ...
    // Везунчик (получила наследство)
    if (!achievements[6].achieved && stats.total_events_impact < -50000) {
        achievements[6].achieved = 1;
        printf("\n  ДОСТИЖЕНИЕ: %s", achievements[6].name);
        printf("\n   %s\n", achievements[6].description);
    }
    
    // Старожил (больше 25 лет)
    if (!achievements[7].achieved && years_passed >= 25) {
        achievements[7].achieved = 1;
        printf("\n ДОСТИЖЕНИЕ: %s", achievements[7].name);
        printf("\n   %s\n", achievements[7].description);
    }
    
    
    //  Вкладчик (сбережения больше 500 000)
    if (!achievements[8].achieved && p->bank.savings >= 500000) {
        achievements[8].achieved = 1;
        printf("\n ДОСТИЖЕНИЕ: %s", achievements[8].name);
...
void print_achievements() {
    ...
    for (int i = 0; i < achievements_count; i++) {
        if (achievements[i].achieved) {
            printf(" + Выполнено ;) - %s (%s)\n", achievements[i].name, achievements[i].description);
            earned_count++;
        } else {
            printf(" - Провалено :( - %s (%s)\n", achievements[i].name, achievements[i].description);
        }
    }
    
    printf("\n СТАТИСТИКА ДОСТИЖЕНИЙ:\n");
    printf("  Получено: %d из %d достижений\n", earned_count, achievements_count);
    
    if (achievements_count > 0) {
        float percent = (float)earned_count / achievements_count * 100;
        printf("  Прогресс: %.0f%%\n", percent);
    }
}

void alice_simulation(int sim_years)
{
    int start_year = 2026;
    int start_month = 2;
    ...
    int end_year = start_year + sim_years;
    ...
    printf(" СИМУЛЯЦИЯ НА %d %s: %d.%d - %d.%d\n\n", 
           sim_years, 
           sim_years == 1 ? "ГОД" : (sim_years < 5 ? "ГОДА" : "ЛЕТ"),
           start_month, start_year, start_month, end_year);
    ...
    while (!(year == end_year && month == start_month)) {
        ...
        check_achievements(&alice, year, month);
...
void print_results()
{
    int total_months = total_simulation_months;
    if (total_months < 1) total_months = 12;
    
    int total_years = total_months / 12;
    ...
    RUB avg_food_cost = stats.food_paid / total_months;  
    ...
    printf("      Период: Февраль 2026 - %d год(а)\n", total_years);
    printf("============================================================\n");
...
int main()
{
    ...
    int years;
    printf("Сколько лет симулировать? ");
    scanf("%d", &years);

    //Ограничиваем не более 49 лет, чтобы компьютер не сошёл с ума
    if (years < 1) years = 1;
    if (years > 50) {
        printf("Слишком много лет! Симулируем 10 лет.\n");
        years = 10;
    }

    alice_init();
    alice_simulation(years);
    print_results();
