...
struct Work {
    ...
    RUB education_allowance_year;
    RUB transport_compensation_month;
    RUB phone_compensation_month;
    RUB stock_options;
    RUB stock_price;
};
...
struct Person {
    ...
    vector<RUB> balance_history;
    vector<RUB> deposit_history;
    vector<RUB> networth_history;
};
...
void alice_salary(int month, int year) {
    if (month == 1 && year == 2027) Alice.work.salary_month = Alice.work.salary_month * 1.05;
    if (month == 1 && year == 2028) Alice.work.salary_month = Alice.work.salary_month * 1.04;
    if (month == 1 && year == 2029) Alice.work.salary_month = Alice.work.salary_month * 1.06;
    if (month == 6 && year == 2026) Alice.bank.balance += Alice.work.salary_month * 0.5;
    if (month == 12 && year == 2026) Alice.bank.balance += Alice.work.salary_month * 0.3;
    if (month == 6 && year == 2027) Alice.bank.balance += Alice.work.salary_month * 0.5;
    if (month == 12 && year == 2027) Alice.bank.balance += Alice.work.salary_month * 0.4;
    if (month == 6 && year == 2028) Alice.bank.balance += Alice.work.salary_month * 0.6;
    if (month == 12 && year == 2028) Alice.bank.balance += Alice.work.salary_month * 0.4;
    if (month == 6 && year == 2029) Alice.bank.balance += Alice.work.salary_month * 0.7;
    if (month == 12 && year == 2029) Alice.bank.balance += Alice.work.salary_month * 0.5;
    ...
    // Опционы (раз в год)
    if (month == 3 && year == 2027) Alice.bank.balance += Alice.work.stock_options * Alice.work.stock_price;
    if (month == 3 && year == 2028) Alice.bank.balance += Alice.work.stock_options * Alice.work.stock_price * 1.1;
    if (month == 3 && year == 2029) Alice.bank.balance += Alice.work.stock_options * Alice.work.stock_price * 1.2;
    Alice.work.experience_years++;
}


void bob_salary(int month, int year) {
    if (month == 1 && year == 2027) Bob.work.salary_month = Bob.work.salary_month * 1.03;
    if (month == 1 && year == 2028) Bob.work.salary_month = Bob.work.salary_month * 1.02;
    if (month == 1 && year == 2029) Bob.work.salary_month = Bob.work.salary_month * 1.04;
    if (month == 1 && year == 2030) Bob.work.salary_month = Bob.work.salary_month * 1.03;
    if (month == 3 && year == 2027) Bob.bank.balance += Bob.work.salary_month * 0.2;
    if (month == 9 && year == 2027) Bob.bank.balance += Bob.work.salary_month * 0.3;
    if (month == 3 && year == 2028) Bob.bank.balance += Bob.work.salary_month * 0.25;
    if (month == 9 && year == 2028) Bob.bank.balance += Bob.work.salary_month * 0.35;
    if (month == 3 && year == 2029) Bob.bank.balance += Bob.work.salary_month * 0.3;
    if (month == 9 && year == 2029) Bob.bank.balance += Bob.work.salary_month * 0.4;
    ...
    if (month == 5 && year == 2027) Bob.bank.balance += Bob.work.stock_options * Bob.work.stock_price;
    if (month == 5 && year == 2028) Bob.bank.balance += Bob.work.stock_options * Bob.work.stock_price * 1.05;
    if (month == 5 && year == 2029) Bob.bank.balance += Bob.work.stock_options * Bob.work.stock_price * 1.1;
...
void alice_additional_income(int month, int year) {
    ...
    // Дивиденды раз в год
    if (month == 4 && year == 2027) Alice.bank.balance += Alice.dividend_income_year;
    if (month == 4 && year == 2028) Alice.bank.balance += Alice.dividend_income_year * 1.02;
    if (month == 4 && year == 2029) Alice.bank.balance += Alice.dividend_income_year * 1.04;
}
...
void bob_additional_income(int month, int year) {
    ...
    if (month == 6 && year == 2027) Bob.bank.balance += Bob.dividend_income_year;
    if (month == 6 && year == 2028) Bob.bank.balance += Bob.dividend_income_year * 1.01;
}
...
void alice_deposit() {
    ...
    if (Alice.bank.pension > 0) {
        Alice.bank.pension += Alice.bank.pension * (0.5 / 12 / 100); // маленький рост
    }
    // Кэшбэк
...
void alice_loan_payments() {
    ...
    if (Alice.car_loan > 0) {
        Alice.bank.balance -= Alice.car_loan_month;
        Alice.car_loan -= Alice.car_loan_month;
    }
...
void bob_loan_payments() {
    ...
    if (Bob.car_loan > 0) {
        Bob.bank.balance -= Bob.car_loan_month;
        Bob.car_loan -= Bob.car_loan_month;
    }
}
...
void alice_cat(int month) {
    ...
    if (Alice.cat.sick) {
        ...
        if (Alice.cat.sick_days <= 0) Alice.cat.sick = false;
    } else {
...
void bob_cat() {
    ...
    if (Bob.cat.sick) {
        ...
        if (Bob.cat.sick_days <= 0) Bob.cat.sick = false;
    } else {
...
void alice_health() {
    ...
    if (random_event(0.3)) Alice.bank.balance -= apply_monthly_inflation(Alice.health.pharmacy, inf);
    if (random_event(0.1)) Alice.bank.balance -= apply_monthly_inflation(Alice.health.dentist, inf);
    if (random_event(0.05)) Alice.bank.balance -= apply_monthly_inflation(Alice.health.therapist, inf);
    if (random_event(0.02)) Alice.bank.balance -= apply_monthly_inflation(Alice.health.cardiologist, inf);
    if (random_event(0.01)) Alice.bank.balance -= apply_monthly_inflation(Alice.health.psychologist, inf);
    if (random_event(0.2)) Alice.bank.balance -= apply_monthly_inflation(Alice.health.massage, inf);
    ...
    if (random_event(0.25)) Alice.bank.balance -= apply_monthly_inflation(Alice.health.barber, inf);
    if (random_event(0.1)) Alice.bank.balance -= apply_monthly_inflation(Alice.health.cosmetics, inf);
    if (random_event(0.05)) Alice.bank.balance -= apply_monthly_inflation(Alice.health.perfume, inf);
    if (random_event(0.03)) Alice.bank.balance -= apply_monthly_inflation(Alice.health.ophthalmologist, inf);
    if (random_event(0.01)) Alice.bank.balance -= apply_monthly_inflation(Alice.health.surgeon, inf);
    if (random_event(0.005)) Alice.bank.balance -= apply_monthly_inflation(Alice.health.hospital, inf);
    if (random_event(0.1)) Alice.bank.balance -= apply_monthly_inflation(Alice.health.physiotherapy, inf);
    ...
    if (random_event(0.2)) Alice.bank.balance -= apply_monthly_inflation(Alice.health.medical_tests, inf);
    if (random_event(0.02)) Alice.bank.balance -= apply_monthly_inflation(Alice.health.ambulance, inf);
}

void bob_health() {
    double inf = random_inflation(7.1, 8.3);
    Bob.bank.balance -= apply_monthly_inflation(Bob.health.medicine, inf);
    if (random_event(0.2)) Bob.bank.balance -= apply_monthly_inflation(Bob.health.pharmacy, inf);
    if (random_event(0.05)) Bob.bank.balance -= apply_monthly_inflation(Bob.health.dentist, inf);
    if (random_event(0.03)) Bob.bank.balance -= apply_monthly_inflation(Bob.health.therapist, inf);
    if (random_event(0.01)) Bob.bank.balance -= apply_monthly_inflation(Bob.health.cardiologist, inf);
    Bob.bank.balance -= apply_monthly_inflation(Bob.health.fitness, inf);
    if (random_event(0.2)) Bob.bank.balance -= apply_monthly_inflation(Bob.health.barber, inf);
    if (random_event(0.02)) Bob.bank.balance -= apply_monthly_inflation(Bob.health.ophthalmologist, inf);
...
void alice_transport() {
    ...
    if (random_event(0.3)) Alice.bank.balance -= apply_monthly_inflation(Alice.transport.taxi, inf);
    if (random_event(0.1)) Alice.bank.balance -= apply_monthly_inflation(Alice.transport.taxi_work, inf);
    if (random_event(0.2)) Alice.bank.balance -= apply_monthly_inflation(Alice.transport.taxi_weekend, inf);
    if (random_event(0.05)) Alice.bank.balance -= apply_monthly_inflation(Alice.transport.train, inf);
    if (random_event(0.02)) Alice.bank.balance -= apply_monthly_inflation(Alice.transport.plane, inf);
    if (random_event(0.03)) Alice.bank.balance -= apply_monthly_inflation(Alice.transport.bus, inf);
    if (random_event(0.1)) Alice.bank.balance -= apply_monthly_inflation(Alice.transport.subway, inf);
    if (random_event(0.05)) Alice.bank.balance -= apply_monthly_inflation(Alice.transport.bicycle, inf);
    if (random_event(0.02)) Alice.bank.balance -= apply_monthly_inflation(Alice.transport.scooter, inf);
    if (random_event(0.1)) Alice.bank.balance -= apply_monthly_inflation(Alice.transport.rideshare, inf);
    if (random_event(0.05)) Alice.bank.balance -= apply_monthly_inflation(Alice.transport.car_sharing, inf);
}

void bob_transport() {
    double inf = random_inflation(7.1, 8.3);
    Bob.bank.balance -= apply_monthly_inflation(Bob.transport.public_transport, inf);
    if (random_event(0.2)) Bob.bank.balance -= apply_monthly_inflation(Bob.transport.taxi, inf);
    if (random_event(0.05)) Bob.bank.balance -= apply_monthly_inflation(Bob.transport.taxi_work, inf);
    if (random_event(0.1)) Bob.bank.balance -= apply_monthly_inflation(Bob.transport.taxi_weekend, inf);
    if (random_event(0.02)) Bob.bank.balance -= apply_monthly_inflation(Bob.transport.train, inf);
    if (random_event(0.01)) Bob.bank.balance -= apply_monthly_inflation(Bob.transport.plane, inf);
    if (random_event(0.05)) Bob.bank.balance -= apply_monthly_inflation(Bob.transport.subway, inf);
}

void alice_clothing() {
    double inf = random_inflation(8.5, 9.7);
    if (random_event(0.2)) Alice.bank.balance -= apply_monthly_inflation(Alice.clothing.clothes, inf);
    if (random_event(0.15)) Alice.bank.balance -= apply_monthly_inflation(Alice.clothing.shoes, inf);
    if (random_event(0.1)) Alice.bank.balance -= apply_monthly_inflation(Alice.clothing.accessories, inf);
    if (random_event(0.1)) Alice.bank.balance -= apply_monthly_inflation(Alice.clothing.dry_clean, inf);
    if (random_event(0.05)) Alice.bank.balance -= apply_monthly_inflation(Alice.clothing.repair, inf);
    if (random_event(0.1)) Alice.bank.balance -= apply_monthly_inflation(Alice.clothing.sportswear, inf);
    if (random_event(0.2)) Alice.bank.balance -= apply_monthly_inflation(Alice.clothing.underwear, inf);
    if (random_event(0.1)) Alice.bank.balance -= apply_monthly_inflation(Alice.clothing.socks, inf);
    if (random_event(0.05)) Alice.bank.balance -= apply_monthly_inflation(Alice.clothing.hats, inf);
    if (random_event(0.02)) Alice.bank.balance -= apply_monthly_inflation(Alice.clothing.gloves, inf);
    if (random_event(0.03)) Alice.bank.balance -= apply_monthly_inflation(Alice.clothing.bags, inf);
    if (random_event(0.01)) Alice.bank.balance -= apply_monthly_inflation(Alice.clothing.jewelry, inf);
    if (random_event(0.02)) Alice.bank.balance -= apply_monthly_inflation(Alice.clothing.watches, inf);
}

void bob_clothing() {
    double inf = random_inflation(7.1, 8.3);
    if (random_event(0.15)) Bob.bank.balance -= apply_monthly_inflation(Bob.clothing.clothes, inf);
    if (random_event(0.1)) Bob.bank.balance -= apply_monthly_inflation(Bob.clothing.shoes, inf);
    if (random_event(0.05)) Bob.bank.balance -= apply_monthly_inflation(Bob.clothing.accessories, inf);
    if (random_event(0.05)) Bob.bank.balance -= apply_monthly_inflation(Bob.clothing.dry_clean, inf);
    if (random_event(0.1)) Bob.bank.balance -= apply_monthly_inflation(Bob.clothing.underwear, inf);
    if (random_event(0.05)) Bob.bank.balance -= apply_monthly_inflation(Bob.clothing.socks, inf);
}
...
void alice_entertainment(int month) {
    ...
    if (month == 12) {
        Alice.bank.balance -= apply_yearly_inflation(Alice.entertainment.travel * 0.5, inf); // зимний отпуск
    }
    if (random_event(0.2)) Alice.bank.balance -= apply_monthly_inflation(Alice.entertainment.museum, inf);
    if (random_event(0.1)) Alice.bank.balance -= apply_monthly_inflation(Alice.entertainment.theater, inf);
    if (random_event(0.05)) Alice.bank.balance -= apply_monthly_inflation(Alice.entertainment.concert, inf);
    if (random_event(0.3)) Alice.bank.balance -= apply_monthly_inflation(Alice.entertainment.cinema, inf);
    ...
    if (random_event(0.2)) Alice.bank.balance -= apply_monthly_inflation(Alice.entertainment.sport, inf);
    if (random_event(0.1)) Alice.bank.balance -= apply_monthly_inflation(Alice.entertainment.souvenirs, inf);
    if (random_event(0.05)) Alice.bank.balance -= apply_monthly_inflation(Alice.entertainment.photos, inf);
    if (random_event(0.1)) Alice.bank.balance -= apply_monthly_inflation(Alice.entertainment.books, inf);
    if (random_event(0.05)) Alice.bank.balance -= apply_monthly_inflation(Alice.entertainment.music, inf);
    if (random_event(0.03)) Alice.bank.balance -= apply_monthly_inflation(Alice.entertainment.games, inf);
    Alice.bank.balance -= apply_monthly_inflation(Alice.entertainment.streaming, inf);
    if (random_event(0.1)) Alice.bank.balance -= apply_monthly_inflation(Alice.entertainment.nightclub, inf);
    if (random_event(0.15)) Alice.bank.balance -= apply_monthly_inflation(Alice.entertainment.bar, inf);
    if (random_event(0.05)) Alice.bank.balance -= apply_monthly_inflation(Alice.entertainment.karaoke, inf);
    if (random_event(0.02)) Alice.bank.balance -= apply_monthly_inflation(Alice.entertainment.bowling, inf);
    if (random_event(0.02)) Alice.bank.balance -= apply_monthly_inflation(Alice.entertainment.billiard, inf);
    if (random_event(0.01)) Alice.bank.balance -= apply_monthly_inflation(Alice.entertainment.quests, inf);
    if (random_event(0.05)) Alice.bank.balance -= apply_monthly_inflation(Alice.entertainment.amusement_park, inf);
    if (random_event(0.02)) Alice.bank.balance -= apply_monthly_inflation(Alice.entertainment.zoo, inf);
    if (random_event(0.01)) Alice.bank.balance -= apply_monthly_inflation(Alice.entertainment.aquarium, inf);
}
...
void bob_entertainment(int month) {
    ...
    if (random_event(0.1)) Bob.bank.balance -= apply_monthly_inflation(Bob.entertainment.museum, inf);
    if (random_event(0.05)) Bob.bank.balance -= apply_monthly_inflation(Bob.entertainment.theater, inf);
    if (random_event(0.2)) Bob.bank.balance -= apply_monthly_inflation(Bob.entertainment.cinema, inf);
    ...
    if (random_event(0.1)) Bob.bank.balance -= apply_monthly_inflation(Bob.entertainment.sport, inf);
    if (random_event(0.05)) Bob.bank.balance -= apply_monthly_inflation(Bob.entertainment.books, inf);
    Bob.bank.balance -= apply_monthly_inflation(Bob.entertainment.streaming, inf);
    if (random_event(0.1)) Bob.bank.balance -= apply_monthly_inflation(Bob.entertainment.bar, inf);
}

void alice_misc() {
    double inf = random_inflation(8.5, 9.7);
    if (random_event(0.1)) Alice.bank.balance -= apply_monthly_inflation(Alice.misc.education, inf);
    if (random_event(0.2)) Alice.bank.balance -= apply_monthly_inflation(Alice.misc.charity, inf);
    if (random_event(0.05)) Alice.bank.balance -= apply_monthly_inflation(Alice.misc.lottery, inf);
    if (random_event(0.02)) Alice.bank.balance -= apply_monthly_inflation(Alice.misc.gambling, inf);
    if (random_event(0.03)) Alice.bank.balance -= apply_monthly_inflation(Alice.misc.fines, inf);
    if (random_event(0.01)) Alice.bank.balance -= apply_monthly_inflation(Alice.misc.taxes, inf);
    if (random_event(0.005)) Alice.bank.balance -= apply_monthly_inflation(Alice.misc.lawyer, inf);
    if (random_event(0.002)) Alice.bank.balance -= apply_monthly_inflation(Alice.misc.notary, inf);
    Alice.bank.balance -= apply_monthly_inflation(Alice.misc.bank_fee, inf);
    if (random_event(0.2)) Alice.bank.balance -= apply_monthly_inflation(Alice.misc.atm_fee, inf);
    if (random_event(0.05)) Alice.bank.balance -= apply_monthly_inflation(Alice.misc.postal, inf);
    if (random_event(0.02)) Alice.bank.balance -= apply_monthly_inflation(Alice.misc.furniture, inf);
    if (random_event(0.03)) Alice.bank.balance -= apply_monthly_inflation(Alice.misc.electronics, inf);
    if (random_event(0.01)) Alice.bank.balance -= apply_monthly_inflation(Alice.misc.home_appliances, inf);
    if (random_event(0.1)) Alice.bank.balance -= apply_monthly_inflation(Alice.misc.kitchen, inf);
    if (random_event(0.15)) Alice.bank.balance -= apply_monthly_inflation(Alice.misc.bed_linen, inf);
    if (random_event(0.2)) Alice.bank.balance -= apply_monthly_inflation(Alice.misc.towels, inf);
    if (random_event(0.1)) Alice.bank.balance -= apply_monthly_inflation(Alice.misc.gifts, inf);
    if (random_event(0.05)) Alice.bank.balance -= apply_monthly_inflation(Alice.misc.flowers, inf);
    if (random_event(0.02)) Alice.bank.balance -= apply_monthly_inflation(Alice.misc.greeting_cards, inf);
    if (random_event(0.01)) Alice.bank.balance -= apply_monthly_inflation(Alice.misc.pet_sitting, inf);
    if (random_event(0.05)) Alice.bank.balance -= apply_monthly_inflation(Alice.misc.house_cleaning, inf);
    if (random_event(0.005)) Alice.bank.balance -= apply_monthly_inflation(Alice.misc.pest_control, inf);
    if (random_event(0.001)) Alice.bank.balance -= apply_monthly_inflation(Alice.misc.moving, inf);
    if (random_event(0.002)) Alice.bank.balance -= apply_monthly_inflation(Alice.misc.storage, inf);
    Alice.bank.balance -= apply_monthly_inflation(Alice.misc.subscriptions_other, inf);
    if (random_event(0.1)) Alice.bank.balance -= apply_monthly_inflation(Alice.misc.software, inf);
    if (random_event(0.05)) Alice.bank.balance -= apply_monthly_inflation(Alice.misc.cloud_storage, inf);
    if (random_event(0.01)) Alice.bank.balance -= apply_monthly_inflation(Alice.misc.domain_hosting, inf);
    if (random_event(0.02)) Alice.bank.balance -= apply_monthly_inflation(Alice.misc.dating_sites, inf);
    if (random_event(0.01)) Alice.bank.balance -= apply_monthly_inflation(Alice.misc.adult_content, inf);
}

void bob_misc() {
    double inf = random_inflation(7.1, 8.3);
    if (random_event(0.05)) Bob.bank.balance -= apply_monthly_inflation(Bob.misc.education, inf);
    if (random_event(0.1)) Bob.bank.balance -= apply_monthly_inflation(Bob.misc.charity, inf);
    if (random_event(0.02)) Bob.bank.balance -= apply_monthly_inflation(Bob.misc.lottery, inf);
    if (random_event(0.01)) Bob.bank.balance -= apply_monthly_inflation(Bob.misc.gambling, inf);
    if (random_event(0.02)) Bob.bank.balance -= apply_monthly_inflation(Bob.misc.fines, inf);
    Bob.bank.balance -= apply_monthly_inflation(Bob.misc.bank_fee, inf);
    if (random_event(0.1)) Bob.bank.balance -= apply_monthly_inflation(Bob.misc.atm_fee, inf);
    if (random_event(0.02)) Bob.bank.balance -= apply_monthly_inflation(Bob.misc.postal, inf);
    if (random_event(0.01)) Bob.bank.balance -= apply_monthly_inflation(Bob.misc.electronics, inf);
    if (random_event(0.05)) Bob.bank.balance -= apply_monthly_inflation(Bob.misc.gifts, inf);
    if (random_event(0.02)) Bob.bank.balance -= apply_monthly_inflation(Bob.misc.flowers, inf);
    Bob.bank.balance -= apply_monthly_inflation(Bob.misc.subscriptions_other, inf);
    if (random_event(0.05)) Bob.bank.balance -= apply_monthly_inflation(Bob.misc.software, inf);
}
...
void alice_personal_events(int month, int year) {
    ...
    if (Alice.has_children) {
        ...
        if (month == Alice.child_birth_month && year == 2027) {
            Alice.bank.balance -= 200000; // расходы на рождение
...
void alice_record_history(int month, int year) {
    ...
    RUB net = Alice.bank.balance + Alice.bank.deposit + Alice.bank.investment + Alice.bank.crypto + Alice.bank.gold + Alice.bank.bonds + Alice.bank.pension + Alice.emergency_fund - Alice.bank.credit_card_debt - Alice.student_loan - Alice.car_loan - Alice.personal_loan - Alice.property.mortgage_debt;
...
void bob_record_history(int month, int year) {
    ...
    RUB net = Bob.bank.balance + Bob.bank.deposit + Bob.bank.investment + Bob.bank.crypto + Bob.bank.gold + Bob.emergency_fund - Bob.bank.credit_card_debt - Bob.student_loan - Bob.car_loan - Bob.property.mortgage_debt;
...
void print_results(const Person &p) {
    ...
    RUB total = p.bank.balance + p.bank.deposit + p.bank.investment + p.bank.crypto + p.bank.gold + p.bank.bonds + p.bank.pension + p.emergency_fund - p.bank.credit_card_debt - p.student_loan - p.car_loan - p.personal_loan - p.property.mortgage_debt;
    printf("Total net worth: %lld RUB\n", total);
    if (p.property.has_mortgage) printf("Mortgage debt: %lld RUB\n", p.property.mortgage_debt);
    if (p.property.owns_apartment) printf("Apartment value: %lld RUB\n", p.property.apartment_value);
    printf("Cat: %s, age %d, color %s\n", p.cat.name.c_str(), p.cat.age, p.cat.color.c_str());
    if (p.has_pet_dog) printf("Has dog, expenses: %lld RUB/month\n", p.dog_expenses_month);
