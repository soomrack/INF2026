...
struct Crisis {
    ...
    int duration;          
    int months_left;       
    RUB salary_multiplier; 
    RUB price_multiplier;  
    char description[50];  
};
...
void alice_init()
{
    ...
    alice.electronics.has_scooter = false;
    ...
    alice.electronics.new_scooter_cost = 75'000;
...
void check_crisis(const int year, const int month) {
    ...
    if (rand() % 1000 < 10) { 
        ...
        alice.crisis.duration = 6 + (rand() % 12); 
        alice.crisis.months_left = alice.crisis.duration;
        
        switch(crisis_type) {
            case 0: 
                alice.crisis.salary_multiplier = 80; // 
                alice.crisis.price_multiplier = 120; // 
                sprintf(alice.crisis.description, "Экономический кризис");
                break;
            case 1: 
                alice.crisis.salary_multiplier = 90; 
                alice.crisis.price_multiplier = 130; 
                sprintf(alice.crisis.description, "Высокая инфляция");
                break;
            case 2: 
                alice.crisis.salary_multiplier = 85; 
                alice.crisis.price_multiplier = 115; 
                sprintf(alice.crisis.description, "Экономическая стагнация");
                break;
            case 3: 
                alice.crisis.salary_multiplier = 75; 
                alice.crisis.price_multiplier = 110;  
                sprintf(alice.crisis.description, "Кризис в отрасли");
                break;
            case 4: 
                alice.crisis.salary_multiplier = 70; 
                alice.crisis.price_multiplier = 140;
...
void alice_buy_electronics(const int year, const int month) {
    ...
    if (rand() % 100 < 5 && alice.capital > 100'000) {
        ...
        const char* items[] = {
            "новый телефон", "новый ноутбук", "новый планшет", "новый телевизор", "новые наушники", "умные часы", "игровую консоль", "новый фотоаппарат", "умный дом", "самокат"
        };
...
void alice_open_business(const int year, const int month) {
    ...
    if (rand() % 100 < 5 && alice.capital >= 100'000) {
        RUB invest = 200'000 + (rand() % 200'000); // 
...
void alice_run_business(const int year, const int month) {
    ...
    if (profit > 100000 && alice.business.months % 12 == 0) {
        alice.business.monthly_income *= 1.1; 
...
void check_and_take_loan(const int year, const int month)
{
    ...
    if (alice.capital <= alice.capital_control.min_threshold) {
        ...
        alice.capital_control.active_loan.interest_rate = 15.0; 
        alice.capital_control.active_loan.months_left = 12; 
...
void process_loan_payment(const int year, const int month)
{
    if (alice.capital_control.active_loan.has_loan) {
        ...
        if (remaining_payment > 0) {
            ...
            RUB interest_part = payment * 0.15; 
...
void alice_investments(const int month, const int year)
{
    ...
    if (alice.capital > 300'000 && rand() % 100 < 10) {
        ...
        printf("Месяц %d: - Инвестиции в крипту: %lld руб. (капитал: %lld руб.)\n", month, crypto_invest, alice.capital);
    }
    

    
    if (alice.investments.crypto > 0) {
        int crypto_change = (rand() % 41) - 20; 
...
void alice_house_repair(const int year) 
{
...
void friends_rest(const int month, const int year) 
{
...
void alice_gambling(const int month, const int year) 
{
...
void print_spending_summary() 
{
    ...
    if (alice.business.exists) {
        ...
        printf("Вложено всего: %lld \n", alice.business.invested);
        printf("Месяцев работы: %d\n", alice.business.months);
        printf("Текущий доход/мес: %lld \n", alice.business.monthly_income);
        printf("Текущие расходы/мес: %lld \n", alice.business.monthly_cost);
    } else if (alice.business.invested > 0) {

        printf(" Бизнес был закрыт или продан\n");
        printf("Всего было вложено: %lld .\n", alice.business.invested);
    ...
    printf(" Самокат: %s\n", alice.electronics.has_scooter ? "есть" : "нет");
    ...
    long long total_expenses = total_food + total_gambling + total_tax + total_fixed_costs + total_car_gas + total_cat_food + total_cat_Vet + total_rest + total_internet + total_clothes + total_gym + total_house_repair + total_child + total_medical + total_entertainment + total_transport + total_education + total_health + total_CounterStrike + total_gifts;
    
    long long total_income = total_salary + (alice.additional_income.freelance * 40) + (alice.additional_income.dividends * 20) + (alice.additional_income.cashback * total_months) + (alice.additional_income.birthday_gifts * 5) + (alice.additional_income.bonus * 5);
