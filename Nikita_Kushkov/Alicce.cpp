...
struct MedicalInsurance {
    ...
    RUB emergency;         // Экстренные случаи (редко)
};

struct Entertainment {
    RUB cinema;            // Кино
    RUB restaurant;        // Рестораны
    RUB subscriptions;     // Подписки (Netflix, Spotify и т.д.)
    RUB hobbies;           // Хобби
};
...
struct Transport {
    ...
    RUB taxi;              // Такси
    RUB car_insurance;     // Страховка машины (раз в год)
    RUB car_maintenance;   // Обслуживание машины
};

struct Education {
    RUB courses;           // Курсы повышения квалификации
    RUB books;             // Книги
    RUB child_circles;     // Кружки для ребенка
};
...
void alice_init()
{
    ...
    alice.investments.deposit = alice.capital; // Депозит
...
void check_and_take_loan(const int year, const int month)
{
    ...
    if (alice.capital <= alice.capital_control.min_threshold) {
        ...
        printf("Капитал упал до %lld руб. (порог %lld руб.)\n", 
               alice.capital - alice.capital_control.active_loan.amount, 
               alice.capital_control.min_threshold);
    ...
    if (alice.capital_control.active_loan.has_loan) {
        ...
        if (remaining_payment > 0) {
            ...
            if (alice.capital_control.active_loan.months_left > 0) {
                double monthly_rate = alice.capital_control.active_loan.interest_rate / 12.0 / 100.0;
                double annuity_factor = (monthly_rate * my_pow(1 + monthly_rate, 
                                       alice.capital_control.active_loan.months_left)) / 
                                       (my_pow(1 + monthly_rate, alice.capital_control.active_loan.months_left) - 1);
                alice.capital_control.active_loan.monthly_payment = 
                    alice.capital_control.active_loan.amount * annuity_factor;
            }
...
// НОВЫЕ ФУНКЦИИ ДЛЯ ДОПОЛНИТЕЛЬНЫХ РАСХОДОВ
...
// НОВЫЕ ФУНКЦИИ ДЛЯ ДОПОЛНИТЕЛЬНЫХ ДОХОДОВ
...
{
    RUB total_child = alice.child.clothes + alice.child.food + 
                      alice.child.medicine + alice.child.school;
...
void alice_casino(const int month, const int year) // Казино
{
    ...
    printf("Месяц %d.%d - Казино: Ставка = %lld руб., число Алисы = %d, ", 
           month, year, bet, alice_number);
...
void alice_simulation()
{
    ...
    while (!(year == 2031 && month == 2)) {
        ...
        alice_gambling(month, year);                   // Лотереи
...
void print_spending_summary() 
{
    ...
    // Новые расходы
    long long total_medical = (alice.medical.monthly * total_months) + 
                               (alice.medical.dentist * 10) + // 2 раза в год * 5 лет
                               (alice.medical.emergency * 3);  // Примерно 3 раза за 5 лет
    
    long long total_entertainment = (alice.entertainment.cinema + 
                                     alice.entertainment.subscriptions) * total_months +
                                     (alice.entertainment.restaurant * 40) + // 70% от месяцев
                                     (alice.entertainment.hobbies * 30);     // 50% от месяцев
    
    long long total_transport = (alice.transport.public_transport * total_months) +
                                 (alice.transport.taxi * 24) + // 40% от месяцев
                                 (alice.transport.car_insurance * 5) +
                                 (alice.transport.car_maintenance * 5);
    
    long long total_education = (alice.education.books * total_months) +
                                 (alice.education.child_circles * total_months) +
                                 (alice.education.courses * 15); // Каждые 4 месяца
    
    long long total_health = (alice.health.pharmacy + alice.health.vitamins) * total_months +
                              (alice.health.sports_equipment * 20); // Каждые 3 месяца
    
    long long total_gifts = (alice.gifts.birthday * 20) + // 4 раза в год * 5 лет
                             (alice.gifts.new_year * 5) +
                             (alice.gifts.other * 18); // 30% от месяцев
    
    long long total_child = (alice.child.clothes + alice.child.food + 
                            alice.child.medicine + alice.child.school) * total_months;
    ...
    printf("Всего инвестиций: %'lld руб.\n", 
           alice.investments.stocks + alice.investments.crypto + alice.capital);
    
    long long total_expenses = total_food + total_gambling + total_tax + total_fixed_costs + 
                               total_car_gas + total_cat_food + total_rest + total_internet + 
                               total_clothes + total_gym + total_house_repair + total_child +
                               total_medical + total_entertainment + total_transport + 
                               total_education + total_health + total_gifts;
    
    long long total_income = total_salary + total_alimony + 
                             (alice.additional_income.freelance * 40) + // 70% от месяцев
                             (alice.additional_income.dividends * 20) + // 4 раза в год * 5 лет
