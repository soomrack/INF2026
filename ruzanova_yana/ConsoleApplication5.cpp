...
void alice_food(const int month, const int year)
{
    ...
    for (int i = 0; i < 15; i++){
        alice_fields[i] = alice_fields[i] + (alice_fields[i] * monthly_inflation);
    }
    ...
    if (alice.married == true){
        ...
        for (int i = 0; i < 15; i++){
            husband_fields[i] = husband_fields[i] + (husband_fields[i] * monthly_inflation);
...
void alice_flat_1(const int month, const int year)
{
    ...
    alice.flat_1.housing_and_communal_servisec = alice.flat_1.housing_and_communal_servisec + (alice.flat_1.housing_and_communal_servisec * (inflation / 100 / 12));

    alice.vtb.account_rub = alice.vtb.account_rub - alice.flat_1.housing_and_communal_servisec;
    alice.vtb.account_rub = alice.vtb.account_rub - alice.flat_1.current_repairs;
    alice.vtb.account_rub = alice.vtb.account_rub - alice.flat_1.household_chemicals;
    alice.vtb.account_rub = alice.vtb.account_rub - alice.flat_1.general_household_needs;
    alice.vtb.account_rub = alice.vtb.account_rub - alice.flat_1.internet;
    alice.vtb.account_rub = alice.vtb.account_rub - alice.flat_1.phone;
    alice.vtb.account_rub = alice.vtb.account_rub - alice.flat_1.cleaning_equipment;
    alice.vtb.account_rub = alice.vtb.account_rub - alice.flat_1.emergency_fund;

    if ((year % 2) == 0){
        alice.vtb.account_rub = alice.vtb.account_rub - alice.flat_1.checking_counters;
    ...
    if ((month % 5) == 0){
        alice.vtb.account_rub = alice.vtb.account_rub - alice.flat_1.repair_of_large_household_appliances;
    }

    if ((month % 3) == 0){
        alice.vtb.account_rub = alice.vtb.account_rub - alice.flat_1.water_filter;
    }
}
...
{
    alice.vtb.account_rub = alice.vtb.account_rub - car.gas;
    alice.vtb.account_rub = alice.vtb.account_rub - car.car_wash;
    alice.vtb.account_rub = alice.vtb.account_rub - car.parking;

    if (month == 10){
        alice.vtb.account_rub = alice.vtb.account_rub - car.winter_tires;
        alice.vtb.account_rub = alice.vtb.account_rub - car.tire_change;
    }
    if (month == 4){
        alice.vtb.account_rub = alice.vtb.account_rub - car.summer_tires;
        alice.vtb.account_rub = alice.vtb.account_rub - car.tire_change;
    }

    if (month == 1){
        alice.vtb.account_rub = alice.vtb.account_rub - car.insurance;
        alice.vtb.account_rub = alice.vtb.account_rub - car.tax;
    }

    if ((month % 3) == 0){
        alice.vtb.account_rub = alice.vtb.account_rub - car.maintenance;
        alice.vtb.account_rub = alice.vtb.account_rub - car.oil_change;
    }

    if ((month % 6) == 0){
        alice.vtb.account_rub = alice.vtb.account_rub - car.diagnostics;
    }
}

void alice_bank_fees(const int month, const int year)
{
    alice.vtb.account_rub = alice.vtb.account_rub - alice.vtb.sms_banking_fee;

    if ((rand() % 100) < 30){
        alice.vtb.account_rub = alice.vtb.account_rub - alice.vtb.transfer_fees;
    }
    ...
    if (alice.vtb.account_usd > 0 && (month % 3) == 0){
        ...
        alice.vtb.account_rub = alice.vtb.account_rub - alice.vtb.currency_exchange_loss;
    }

    if ((rand() % 100) < 20){
        alice.vtb.account_rub = alice.vtb.account_rub - alice.vtb.atm_withdrawal_fee;
    }
}
...
void alice_subscriptions(const int month, const int year)
{
    ...
    RUB total_subs = 0;
    total_subs = total_subs + alice.subs.netflix;
    total_subs = total_subs + alice.subs.spotify;
    total_subs = total_subs + alice.subs.youtube_premium;
    total_subs = total_subs + alice.subs.icloud;
    total_subs = total_subs + alice.subs.dropbox;
    total_subs = total_subs + alice.subs.adobe_cloud;
    total_subs = total_subs + alice.subs.chatgpt_plus;
    total_subs = total_subs + alice.subs.online_cinema;
    total_subs = total_subs + alice.subs.fitness_app;
    total_subs = total_subs + alice.subs.password_manager;
    total_subs = total_subs + alice.subs.vpn_service;
    total_subs = total_subs + alice.subs.alarm_clock_app;
...
void alice_mortgage(const int month, const int year)
{
    ...
    if (mortgage_taken == false && alice.married == true && year == 2027 && month == 7){
        alice.vtb.account_rub = alice.vtb.account_rub - alice.flat_2.down_payment;
        alice.vtb.account_rub = alice.vtb.account_rub - alice.flat_2.appraisal;
        alice.vtb.account_rub = alice.vtb.account_rub - alice.flat_2.notary;
        alice.vtb.account_rub = alice.vtb.account_rub - alice.flat_2.state_fee;
        alice.vtb.account_rub = alice.vtb.account_rub - alice.flat_2.bank_commission;
        mortgage_taken = true;
    }

    if (mortgage_taken == true){
        alice.vtb.account_rub = alice.vtb.account_rub - alice.flat_2.monthly_payment;
        alice.flat_2.loan_amount = alice.flat_2.loan_amount - alice.flat_2.monthly_payment;

        RUB interest = static_cast<RUB>(alice.flat_2.loan_amount * (alice.flat_2.interest_rate / 100.0 / 12.0));
        alice.vtb.account_rub = alice.vtb.account_rub - interest;

        if (month == 7){
            alice.vtb.account_rub = alice.vtb.account_rub - alice.flat_2.property_insurance;
            alice.vtb.account_rub = alice.vtb.account_rub - alice.flat_2.life_insurance;
        }

        int renovation_months[] = { 8, 9, 10, 11, 12 };
        for (int i = 0; i < 5; i++){
            if (year == 2027 && month == renovation_months[i]){
                alice.vtb.account_rub = alice.vtb.account_rub - (alice.flat_2.renovation / 6);
            }
        }
        ...
        if ((year == 2027 && month == 12) || (year == 2028 && month == 1)){
            alice.vtb.account_rub = alice.vtb.account_rub - (alice.flat_2.furniture / 2);
        }
    }
}
...
void alice_wedding(const int month, const int year)
{
    if (alice.wedding.happened == false && year == 2026 && month == 8){
        ...
        total = total + alice.wedding.restaurant;
        total = total + alice.wedding.venue_decor;
        total = total + alice.wedding.wedding_cake;
        total = total + alice.wedding.alcohol;

        total = total + alice.wedding.dress_and_suit;
        total = total + alice.wedding.makeup_artist;
        total = total + alice.wedding.hairdresser;
        total = total + alice.wedding.bouquet;
        total = total + alice.wedding.boutonniere;

        total = total + alice.wedding.rings;
        total = total + alice.wedding.invitations;

        total = total + alice.wedding.photographer;
        total = total + alice.wedding.videographer;
        total = total + alice.wedding.music;
        total = total + alice.wedding.host;

        total = total + alice.wedding.transport;
        total = total + alice.wedding.honeymoon;

        total = total + alice.wedding.other_expenses;
...
void alice_child(const int month, const int year)
{
    ...
    if (alice.child.born == true) {
        ...
        alice.vtb.account_rub = alice.vtb.account_rub - alice.child.monthly_food;
        alice.vtb.account_rub = alice.vtb.account_rub - alice.child.diapers;
        alice.vtb.account_rub = alice.vtb.account_rub - alice.child.clothes;
        alice.vtb.account_rub = alice.vtb.account_rub - alice.child.medicine;
        alice.vtb.account_rub = alice.vtb.account_rub - alice.child.toys;
        alice.vtb.account_rub = alice.vtb.account_rub - alice.child.hygiene;

        if (alice.child.age_months <= 12){
            if ((month % 3) == 0){
                alice.vtb.account_rub = alice.vtb.account_rub - alice.child.swimming;
                alice.vtb.account_rub = alice.vtb.account_rub - alice.child.massage;
            }
            alice.vtb.account_rub = alice.vtb.account_rub - (alice.child.breastfeeding / 3);
        }
        ...
        if (alice.child.age_months >= 12){
            ...
            alice.vtb.account_rub = alice.vtb.account_rub - alice.child.development;
            alice.vtb.account_rub = alice.vtb.account_rub - (alice.child.car_seat / 12);
        }

        if (alice.child.age_months >= 18){
            alice.vtb.account_rub = alice.vtb.account_rub - alice.child.kindergarten;
        }

        if (alice.child.age_months >= 24){
            if ((month % 6) == 0){
                alice.vtb.account_rub = alice.vtb.account_rub - alice.child.dentist;
            }
        }

        if ((month % 2) == 0){
            alice.vtb.account_rub = alice.vtb.account_rub - alice.child.doctor_visits;
            alice.vtb.account_rub = alice.vtb.account_rub - alice.child.vitamins;
        }

        if (month == 5){
            alice.vtb.account_rub = alice.vtb.account_rub - alice.child.child_birthday;
        }

        if (alice.vtb.account_rub > 200'000 && (rand() % 10) == 0){
            alice.vtb.account_rub = alice.vtb.account_rub - alice.child.babysitter;
        }

        if (alice.child.age_months == 12){
            alice.vtb.account_rub = alice.vtb.account_rub - alice.child.high_chair;
        }
        if (alice.child.age_months == 18){
            alice.vtb.account_rub = alice.vtb.account_rub - alice.child.playpen;
        }
    }
}
...
void alice_divorce(const int month, const int year)
{
    if (alice.divorce.happened == false && alice.married == true && alice.child.born == true && year == 2029 && month == 3 && alice.vtb.account_rub < 100'000){
        RUB total_divorce = alice.divorce.lawyer_services + alice.divorce.court_fee + alice.divorce.appraiser_services + alice.divorce.notary_for_divorce;
...
void alice_job_change(const int month, const int year)
{
    ...
    if (alice.alice_job.employed == false && alice.alice_job.looking_for_job == true){
        ...
        if ((month % 3) == 0){
            alice.vtb.account_rub = alice.vtb.account_rub - (alice.alice_job.training_courses / 3);
        }
...
void alice_health_addiction(const int month, const int year)
{
    ...
    monthly_supplements = monthly_supplements + alice.health.protein;
    monthly_supplements = monthly_supplements + alice.health.bcaa;
    monthly_supplements = monthly_supplements + alice.health.creatine;
    monthly_supplements = monthly_supplements + alice.health.pre_workout;
    monthly_supplements = monthly_supplements + alice.health.vitamins_sports;

    if (alice.health.addiction_level > 50){
        monthly_supplements = monthly_supplements + alice.health.omega3;
        monthly_supplements = monthly_supplements + alice.health.collagen;
        monthly_supplements = monthly_supplements + alice.health.glutamine;
    }

    RUB organic_food = 0;
    organic_food = organic_food + alice.health.organic_vegetables;
    organic_food = organic_food + alice.health.organic_meat;
    organic_food = organic_food + alice.health.organic_chicken;
    organic_food = organic_food + alice.health.organic_eggs;
    organic_food = organic_food + alice.health.organic_fish;
    organic_food = organic_food + alice.health.organic_dairy;

    if (alice.health.orthorexia == true){
        organic_food = organic_food + alice.health.superfoods;
        organic_food = organic_food + alice.health.gluten_free;
        organic_food = organic_food + alice.health.lactose_free;
        organic_food = organic_food + alice.health.healthy_snacks;
        organic_food = organic_food + alice.health.detox_water;
    }
...
void alice_sleep_health(const int month, const int year)
{
    if (month == 11 && year == 2027){
        alice.vtb.account_rub = alice.vtb.account_rub - alice.sleep.orthopedic_mattress;
        alice.vtb.account_rub = alice.vtb.account_rub - alice.sleep.expensive_pillows;
    }
    if (month == 10 && year == 2028){
        alice.vtb.account_rub = alice.vtb.account_rub - alice.sleep.weighted_blanket;
        alice.vtb.account_rub = alice.vtb.account_rub - alice.sleep.blackout_curtains;
    }
    if (month >= 10 || month <= 3){
        alice.vtb.account_rub = alice.vtb.account_rub - alice.sleep.vitamin_d;
    }
    if (month == 1 && alice.married == true && year >= 2027){
        alice.vtb.account_rub = alice.vtb.account_rub - alice.sleep.winter_sun_trips;
    }
}
...
{
    RUB apps_total = 0;
    apps_total = apps_total + alice.paid_apps.planner_app;
    apps_total = apps_total + alice.paid_apps.password_manager_app;
    apps_total = apps_total + alice.paid_apps.vpn_app;
    apps_total = apps_total + alice.paid_apps.sleep_app;
    apps_total = apps_total + alice.paid_apps.meditation_app;
    apps_total = apps_total + alice.paid_apps.fitness_app_paid;
    apps_total = apps_total + alice.paid_apps.language_learning;
    apps_total = apps_total + alice.paid_apps.productivity_tools;
...
void alice_emotional_spending(const int month, const int year)
{
    ...
    if (emotional_total > 0){
        alice.emotions.unused_items = alice.emotions.unused_items + (emotional_total * 0.3);
    }
}
...
void alice_handle_pets(const int month, const int year)
{
    ...
    if (alice.cat.have_pet == true){
        alice.vtb.account_rub = alice.vtb.account_rub - alice.cat.food;
        alice.vtb.account_rub = alice.vtb.account_rub - alice.cat.treats;
        alice.vtb.account_rub = alice.vtb.account_rub - alice.cat.filler;
        alice.vtb.account_rub = alice.vtb.account_rub - (alice.cat.toys_for_pet / 3);

        if ((month % 3) == 0){
            alice.vtb.account_rub = alice.vtb.account_rub - alice.cat.veterinary;
            alice.vtb.account_rub = alice.vtb.account_rub - alice.cat.grooming;
        }

        if ((month % 6) == 0){
            alice.vtb.account_rub = alice.vtb.account_rub - alice.cat.vaccinations;
            alice.vtb.account_rub = alice.vtb.account_rub - alice.cat.claws_care;
        }

        if (month == 1){
            alice.vtb.account_rub = alice.vtb.account_rub - alice.cat.bedding;
            alice.vtb.account_rub = alice.vtb.account_rub - (alice.cat.carrier / 2);
        }
    }

    if (alice.hamster.have_pet == true){
        alice.vtb.account_rub = alice.vtb.account_rub - alice.hamster.food;
        alice.vtb.account_rub = alice.vtb.account_rub - alice.hamster.treats;
        alice.vtb.account_rub = alice.vtb.account_rub - alice.hamster.filler;
        alice.vtb.account_rub = alice.vtb.account_rub - (alice.hamster.toys_for_pet / 3);

        if ((month % 3) == 0){
            alice.vtb.account_rub = alice.vtb.account_rub - alice.hamster.veterinary;
            alice.vtb.account_rub = alice.vtb.account_rub - alice.hamster.grooming;
        }

        if ((month % 6) == 0){
            alice.vtb.account_rub = alice.vtb.account_rub - alice.hamster.vaccinations;
            alice.vtb.account_rub = alice.vtb.account_rub - alice.hamster.claws_care;
        }

        if (month == 1){
            alice.vtb.account_rub = alice.vtb.account_rub - alice.hamster.bedding;
            alice.vtb.account_rub = alice.vtb.account_rub - (alice.hamster.carrier / 2);
        }
    }

    if (alice.dog.have_pet == true){
        alice.vtb.account_rub = alice.vtb.account_rub - alice.dog.food;
        alice.vtb.account_rub = alice.vtb.account_rub - alice.dog.treats;
        alice.vtb.account_rub = alice.vtb.account_rub - alice.dog.filler;
        alice.vtb.account_rub = alice.vtb.account_rub - (alice.dog.toys_for_pet / 3);

        if ((month % 3) == 0){
            alice.vtb.account_rub = alice.vtb.account_rub - alice.dog.veterinary;
            alice.vtb.account_rub = alice.vtb.account_rub - alice.dog.grooming;
        }

        if ((month % 6) == 0){
            alice.vtb.account_rub = alice.vtb.account_rub - alice.dog.vaccinations;
            alice.vtb.account_rub = alice.vtb.account_rub - alice.dog.claws_care;
        }

        if (month == 1){
            alice.vtb.account_rub = alice.vtb.account_rub - alice.dog.bedding;
            alice.vtb.account_rub = alice.vtb.account_rub - (alice.dog.carrier / 2);
        }
    }
}
...
void print_results()
{
    ...
    printf("\nкапитал:\n");
    printf("наличные/счета: %'lld руб.\n", alice.vtb.account_rub);
    printf("депозиты: %'lld руб.\n", alice.vtb.deposit_rub);
