...
double monthly_inflation_factor(double annual_pp) {
	return 1.0 + annual_pp / 12.0 / 100.0;
}
...
void receive_personalized_salary(Person& p, const char* person_name, int month, int year) {
	// Общая индексация (как и раньше)
	receive_salary(p, month, year);

	// Персонализация под конкретного человека:
	// у Trump с июля 2027 года фиксированная прибавка за проект.
	if (person_name[0] == 'T' && month == 7 && year == 2027) {
		p.bank.account += 30000;
	}
}

void pay_food(Person& p, double annual_inflation_pp) {
	inflate_food(p.food, monthly_inflation_factor(annual_inflation_pp));
	p.bank.account -= total_food(p.food);
}

void pay_subscriptions(Person& p, double annual_inflation_pp) {
	inflate_subscriptions(p.subscriptions, monthly_inflation_factor(annual_inflation_pp));
	p.bank.account -= total_subscriptions(p.subscriptions);
}

void pay_entertainment(Person& p, double annual_inflation_pp) {
	inflate_entertainment(p.entertainment, monthly_inflation_factor(annual_inflation_pp));
	p.bank.account -= total_entertainment(p.entertainment);
}

void pay_transport(Person& p, double annual_inflation_pp) {
	double k = monthly_inflation_factor(annual_inflation_pp);
...
void pay_home(Person& p, double annual_inflation_pp) {
	inflate_home(p.home, monthly_inflation_factor(annual_inflation_pp));
	p.bank.account -= total_home(p.home);
}

void pay_health(Person& p, double annual_inflation_pp) {
	inflate_health(p.health, monthly_inflation_factor(annual_inflation_pp));
	p.bank.account -= total_health(p.health);
}

void pay_clothes(Person& p, double annual_inflation_pp) {
	inflate_clothes(p.clothes, monthly_inflation_factor(annual_inflation_pp));
	p.bank.account -= total_clothes(p.clothes);
}

void pay_education(Person& p, double annual_inflation_pp) {
	inflate_education(p.education, monthly_inflation_factor(annual_inflation_pp));
	p.bank.account -= total_education(p.education);
}

void pay_gifts(Person& p, double annual_inflation_pp) {
	inflate_gifts(p.gifts, monthly_inflation_factor(annual_inflation_pp));
	p.bank.account -= total_gifts(p.gifts);
}

void pay_personal_care(Person& p, double annual_inflation_pp) {
	inflate_personal_care(p.personal_care, monthly_inflation_factor(annual_inflation_pp));
	p.bank.account -= total_personal_care(p.personal_care);
}

void pay_hobbies(Person& p, double annual_inflation_pp) {
	inflate_hobbies(p.hobbies, monthly_inflation_factor(annual_inflation_pp));
	p.bank.account -= total_hobbies(p.hobbies);
}

void pay_all_pets(Person& p, double annual_inflation_pp) {
	double k = monthly_inflation_factor(annual_inflation_pp);
	inflate_dog(p.dog, k);
	inflate_cat(p.cat, k);
...
void process_month(Person* person_ptr, const char* person_name, int month, int year) {
	// Работа через указатель: видно, что функции меняют исходный объект Person
	// person_ptr->поле эквивалентно (*person_ptr).поле, но запись короче и нагляднее
	deposit_interest(*person_ptr, DEPOSIT_RATE);
	pay_food(*person_ptr, INFLATION);
	pay_subscriptions(*person_ptr, INFLATION);
	pay_entertainment(*person_ptr, INFLATION);
	pay_home(*person_ptr, INFLATION);
	pay_health(*person_ptr, INFLATION);
	pay_clothes(*person_ptr, INFLATION);
	pay_education(*person_ptr, INFLATION);
	pay_gifts(*person_ptr, INFLATION);
	pay_personal_care(*person_ptr, INFLATION);
	pay_hobbies(*person_ptr, INFLATION);
	pay_transport(*person_ptr, INFLATION);
	pay_all_pets(*person_ptr, INFLATION);
	transfer_to_deposit(*person_ptr, 50000);
	receive_personalized_salary(*person_ptr, person_name, month, year);
}

void simulation(Person& p, const char* person_name) {
	int month = 10;
	int year = 2026;

	while (!(month == 11 && year == 2027)) {
		process_month(&p, person_name, month, year);
...
PersonConfig make_epstein_config() {
...
PersonConfig make_trump_config() {
...
int main() {
	...
	PersonConfig epstein_cfg = make_epstein_config();
	PersonConfig trump_cfg = make_trump_config();

	person_init(Epstein, epstein_cfg);
	person_init(Trump, trump_cfg);

	simulation(Epstein, "Epstein");
	simulation(Trump, "Trump");
