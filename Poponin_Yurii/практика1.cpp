...
void alice_pay_mortgage(int month, int year);
void init_alice_mortgage();
void alice_housing_expenses(int month, int year);  // функции начинаются одинаково
void alice_extrawork(int month, int year);
...
void bob_extrawork(int month, int year);
void init_bob_mortgage();
void bob_pay_mortgage(int month, int year);
...
void alice_deposit()
{
    if (Alice.vtb.bankcard > 100'000)  // return
    {
...
void alice_food_expenses(int month, int year) { // vitamins 
    RUB grocery_bill = 25000; // везде конст
    RUB cafe_bill = 8000;
...
void alice_car_expenses(int month, int year) {  // tires
...
void alice_lifestyle(int month, int year) { // разделить хорошие и плохие события в разные функции чтобы одновременно могли и хорошие и лпохие происходить
    if ((rand() % 100) > 30) return;
    int good_or_bad_event = rand() % 10;

    if (good_or_bad_event <= 8) {
...
void bob_food_expenses(int month, int year) { // если будет совсем много, то сделать отдельные функции
    RUB fastfood = 25000; 
...
void apply_salary_indexation(int month, int year) {
    if (month != 1) return;
...
void apply_global_inflation(int month, int year) { // сделать отдельно общую инфляцию, независящую от банротства боба или алисы
    float monthly_base = Russia2026.inflation_rate / 12.0;

    // 1. Инфляция для Алисы
    if (Alice.is_bankrupt == 0) {
