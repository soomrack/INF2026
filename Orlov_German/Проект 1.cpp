...
struct World {
    ...
    percent social_deduction_limit;    // max соц. вычет в год
    percent property_deduction_limit;  // max имущ. вычет с покупки (260 тыс. руб.)
    percent interest_deduction_limit;  // max вычет по процентам (390 тыс.)
};
...
void process_tax_deductions(Person& person) {
    ...
    // Подаём декларацию в конце года; считаем, что это происходит в январе следующего
    if (person.cumulative_income_this_year == 0) return;

    RUB total_income = person.cumulative_income_this_year;
    RUB tax_due = calculate_income_tax(total_income);
    ...
    RUB total_social_eligible = std::min(health_spent + sport_spent + education_spent,
        static_cast<RUB>(world.social_deduction_limit));
    RUB social_deduction = total_social_eligible;
    RUB social_refund = static_cast<RUB>(social_deduction * world.income_tax_rate / 100.0);

    // Имущественный вычет за покупку жилья (только если есть ипотека и ещё не использован)
    RUB property_deduction_used_this_year = 0;
    if (person.has_mortgage && !person.mortgage.tax_deduction_used) {
        // вычет даётся с суммы до 2 млн
        ...
        // можем вернуть не больше уплаченного налога
        RUB available_for_refund = tax_already_paid;
        RUB expected_refund = std::min(max_refund, available_for_refund);
        property_deduction_used_this_year = static_cast<RUB>(expected_refund / (world.income_tax_rate / 100.0));
        person.mortgage.tax_deduction_used = true; // используем весь сразу (упрощение)
        person.pending_tax_refund += expected_refund;
        available_for_refund -= expected_refund;
    }

    // Вычет по процентам ипотеки
    RUB interest_deduction_this_year = 0;
    if (person.has_mortgage && person.mortgage.tax_deduction_remaining > 0) {
        RUB interest_paid_this_year = person.mortgage.total_interest_paid; // за весь период, но мы можем накопить
        // реально мы считаем проценты, уплаченные в этом году
        // будем использовать накопленные проценты для вычета
        ...
        RUB available = tax_already_paid - property_deduction_used_this_year * world.income_tax_rate / 100.0 - social_refund;
        RUB interest_refund = std::min(max_interest_refund, available);
    ...
    // Социальный вычет применяем после имущественного, если остался налог
    RUB remaining_tax_for_social = tax_already_paid - (property_deduction_used_this_year * world.income_tax_rate / 100.0)
        - (interest_deduction_this_year * world.income_tax_rate / 100.0);
    social_refund = std::min(social_refund, remaining_tax_for_social);
    if (social_refund > 0) {
    ...
    rec.tax_refund_received = person.pending_tax_refund;
...
void alice_random_bonus() {
    ...
    if ((rand() % 100) < 20) {
        ...
        alice.cash += bonus;
        alice.cumulative_income_this_year += bonus; // облагается
        pay_income_tax_monthly(alice, bonus);
        alice.stress_index -= 2;
    }
}
...
void alice_overtime_pay() {
    ...
    if ((rand() % 100) < 25) {
        ...
        alice.cash += overtime;
        alice.cumulative_income_this_year += overtime;
        pay_income_tax_monthly(alice, overtime);
...
void bob_random_bonus() {
    ...
    if ((rand() % 100) < 20) {
        RUB bonus = 30'000 + rand() % 70'000;
        bob.cash += bonus;
        bob.cumulative_income_this_year += bonus;
        pay_income_tax_monthly(bob, bonus);
        bob.stress_index -= 2;
    }
}
...
void bob_overtime_pay() {
    ...
    if ((rand() % 100) < 25) {
        RUB overtime = 10'000 + rand() % 30'000;
        bob.cash += overtime;
        bob.cumulative_income_this_year += overtime;
        pay_income_tax_monthly(bob, overtime);
        bob.stress_index += 1;
    }
}
...
RUB bob_calculate_net_worth() {
    RUB assets = bob.cash + bob.deposit.amount + bob.land_cruiser.value + bob.flat_to_live.value + bob.stocks.amount;
    RUB liabilities = bob.credit.principal + bob.mortgage.remaining_principal;
    return assets - liabilities;
}
