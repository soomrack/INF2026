...
void init_person(struct Person* person, const char* name, int age, RUB salary, RUB capital) {
    ...
    person->shopping_spending = 2000;
    person->clothing_expenses = 2500;
    person->entertainment_expenses = 3000;
    person->household_expenses = 2500;
    person->transport_expenses = 2000;
...
void init_family() {
    ...
    family.daughter.food = 4500;
    family.daughter.mental_health = 65 + (rand() % 10);
    family.daughter.shopping_spending = 2000;
...
void person_food(struct Person* person) {
    ...
    RUB salary_factor = (person->salary > 0) ? (RUB)((person->salary * 0.035)) : 0;
    RUB actual_food = (RUB)((person->food + salary_factor * 0.35) * (1 + config.inflation_rate / 100.0 / 12.0));
...
void person_clothing(struct Person* person) {
    if (!person->is_alive || person->in_clinic) return;
    RUB salary_factor = (person->salary > 0) ? (RUB)(person->salary * 0.02) : 0;
    RUB actual = (RUB)((person->clothing_expenses + salary_factor * 0.2) * (1 + config.inflation_rate / 100.0 / 12.0));
...
void person_entertainment(struct Person* person) {
    if (!person->is_alive || person->in_clinic) return;
    RUB salary_factor = (person->salary > 0) ? (RUB)(person->salary * 0.025) : 0;
    RUB actual = (RUB)((person->entertainment_expenses + salary_factor * 0.4) * (1 + config.inflation_rate / 100.0 / 12.0));
    family.family_budget -= actual;
    current_stats.expenses += actual;
...
void person_household(struct Person* person) {
    if (!person->is_alive || person->in_clinic) return;
    RUB salary_factor = (person->salary > 0) ? (RUB)(person->salary * 0.015) : 0;
    RUB actual = (RUB)((person->household_expenses + salary_factor * 0.2) * (1 + config.inflation_rate / 100.0 / 12.0));
    family.family_budget -= actual;
    current_stats.expenses += actual;
...
void person_transport(struct Person* person) {
    if (!person->is_alive || person->in_clinic) return;
    RUB salary_factor = (person->salary > 0) ? (RUB)(person->salary * 0.02) : 0;
    RUB actual = (RUB)((person->transport_expenses + salary_factor * 0.2) * (1 + config.inflation_rate / 100.0 / 12.0));
    family.family_budget -= actual;
    current_stats.expenses += actual;
...
int main() {
    ...
    printf("========================================\n");
    printf("   СИМУЛЯТОР СЕМЕЙНЫХ ФИНАНСОВ v1.0  \n");
    printf("========================================\n\n");
