...
const string g_minor_events_positive[] = {
    "Семья нашла кошелек с небольшой суммой",
    "На улице встретили старого друга",
    "Дочь получила похвалу в школе",
    "Погода весь месяц была отличная",
    "Соседи подарили вкусный торт",
    "Удалось купить билеты со скидкой",
    "Хорошие новости от родственников",
    "Случайная находка в кармане",
    "Премия на работе без причины",
    "Бесплатный концерт в парке",
    "Подарок от старых друзей",
    "Выиграли в лотерею мелочь",
    "На дороге нашли кошелек",
    "Получили бесплатный образец",
    "Скидка в любимом магазине",
    "Хороший отзыв о работе",
    "Неожиданный бонус от фирмы",
    "Подарок на день рождения",
    "Приятная встреча в кафе",
    "Бесплатная консультация юриста"
};
const string g_minor_events_negative[] = {
    "Потеряли ключи от квартиры",
    "Сломалась бытовая техника",
    "Порвалась новая одежда",
    "Опоздали на важный автобус",
    "Забыли зонт под дождем",
    "Разбился телефон",
    "Испорчен ужин",
    "Потеряли документы",
    "Конфликт с соседями",
    ...
    "Сломал замок в двери",
    "Пропал домашний",
    "Укусил комар",
    "Сломался каблук",
    "Потеряли кошелек на улице",
    "Испорчен отдых",
    "Ссора с друзьями",
    "Неприятный запах",
    "Сломался компьютер"
...
const string g_minor_events_neutral[] = {
    ...
    "Просмотрели новый фильм",
    "Чтение книги вечером",
    "Прогулка в парке",
    "Встреча с соседом",
    "Покупка продуктов",
    "Звонок от друзей",
...
void print_person_finances(const Person* p) {
    cout << " Баланс счета: " << p->account.balance << " руб. " << endl;
    cout << " Микрозаймы: " << p->account.microloan_debt << " руб. " << endl;
...
void print_family_budget_status() {
    ...
    cout << " Уровень стресса: " << g_family.family_stress << endl;
    ...
    if (g_house.is_owned) {
        cout << " Дом: собственный (" << g_house.address << ") " << endl;
    }
    else {
        cout << " Дом: аренда (" << g_house.rent_cost << " руб./мес) " << endl;
    }
}
...
void print_event_log_entry(const EventLog& ev) {
    ...
    case 2: case 3: type_str = "[НЕЙТРАЛЬНО] "; break;
    case 4: type_str = "[ПЛОХО] "; break;
    case 5: type_str = "[КРИТИЧНО] "; break;
...
void print_month_events() {
    ...
    if (events_found == 0) {
        cout << "  [ИНФО] В этом месяце ничего особенного не произошло" << endl;
    }
...
void print_month_income_section() {
    ...
    cout << "  Бонусы: " << g_current_stats.career_bonus << " руб. " << endl;
}
void print_month_expenses_section() {
    ...
    cout << "  Отпуск/санаторий: " << (g_current_stats.vacation_expenses + g_current_stats.sanatorium_expenses) << " руб. " << endl;
    cout << "  Медицина: " << g_current_stats.medical_expenses << " руб. " << endl;
    cout << "  Микрозаймы: " << g_current_stats.microloan_payments << " руб. " << endl;
    cout << "  Проценты по займам: " << g_current_stats.microloan_interest << " руб. " << endl;
    cout << "  Налоги: " << g_current_stats.tax_payments << " руб. " << endl;
    cout << "  Страховки: " << g_current_stats.insurance_payments << " руб. " << endl;
    cout << "  Непредвиденные: " << g_current_stats.unexpected_expenses << " руб. " << endl;
    cout << "  Потери инвестиций: " << g_current_stats.investment_loss << " руб. " << endl;
}
...
void print_final_family_status() {
    ...
    cout << " Муж: " << (g_family.husband.health.is_alive ? "ЖИВ " : "МЕРТВ ") << endl;
    if (g_family.husband.health.is_alive) {
        cout << "   Микрозаймы: " << g_family.husband.account.microloan_debt << " руб. " << endl;
        cout << "   Всего микрозаймов: " << g_family.husband.account.microloan_count << endl;
        cout << "   Должность: " << g_family.husband.career.position << endl;
    }
    cout << " Жена: " << (g_family.wife.health.is_alive ? "ЖИВА " : "МЕРТВА ") << endl;
    if (g_family.wife.health.is_alive) {
        cout << "   Микрозаймы: " << g_family.wife.account.microloan_debt << " руб. " << endl;
        cout << "   Всего микрозаймов: " << g_family.wife.account.microloan_count << endl;
        cout << "   Должность: " << g_family.wife.career.position << endl;
    }
    cout << " Дочь: " << (g_family.daughter.health.is_alive ? "ЖИВА " : "МЕРТВА ") << endl;
    ...
    cout << " Собака: " << (g_family.dog.is_alive ? "ЖИВА " : "МЕРТВА ") << endl;
    cout << " Кот: " << (g_family.cat.is_alive ? "ЖИВ " : "МЕРТВ ") << endl;
...
void print_final_financial_status() {
    cout << endl;
    cout << "ФИНАНСЫ: " << endl;
    cout << " Остаток на общем счёте: " << g_family.shared_budget_pool << " руб. " << endl;
    cout << " Всего заработано за все время: " << g_state.total_income << " руб. " << endl;
    cout << " Всего потрачено за все время: " << g_state.total_expenses << " руб. " << endl;
...
void print_final_achievements() {
    ...
    if (g_state.bankruptcy_months == 0) {
        cout << " - Семья не стала банкротом " << endl;
    }
    if (g_family.total_microloans_taken == 0) {
        cout << " - Не брали микрозаймы " << endl;
    }
}
void print_event_log_full() {
    print_header("ПОЛНЫЙ ЖУРНАЛ СОБЫТИЙ");
...
void print_minor_events_list() {
    cout << endl;
    cout << "Количество малых событий: " << endl;
    cout << " Положительных: " << sizeof(g_minor_events_positive) / sizeof(g_minor_events_positive[0]) << endl;
    cout << " Отрицательных: " << sizeof(g_minor_events_negative) / sizeof(g_minor_events_negative[0]) << endl;
    cout << " Нейтральных: " << sizeof(g_minor_events_neutral) / sizeof(g_minor_events_neutral[0]) << endl;
    print_separator();
}
...
RUB get_dynamic_price(RUB base_price) {
    PERCENT fluctuation = (rand() % 31 - 15) / 100.0;  
    PERCENT final_multiplier = g_prices.inflation_multiplier * (1.0 + fluctuation);
    return (RUB)(base_price * final_multiplier);
}
...
void init_education() {
    ...
    g_daughter_edu.institution_name = "НЕ ОПРЕДЕЛЕНО";
    g_daughter_edu.years_remaining = 0;
    g_daughter_edu.status = "ШКОЛЬНИК";
...
void process_husband_month() {
    Person* p = &g_family.husband;
    if (!p->health.is_alive) return;

    if (p->career.base_salary > 0 && p->career.can_work && !p->health.in_clinic) {
        ...
        if ((g_current_stats.month == 8) && (g_current_stats.year == 2026) && (p->career.base_salary > 50000)) {
            p->career.base_salary = (RUB)(p->career.base_salary * 1.5);
            add_event(p->name + " получил повышение", 1, "КАРЬЕРА");
        }
        if ((g_current_stats.month == 1) && (g_current_stats.year > 2026)) {
    ...
    if (!p->health.in_clinic) {
        ...
        actual = get_dynamic_price(g_prices.clothing_base);
        process_expense(p, actual, 2);

        actual = get_dynamic_price(g_prices.entertainment_base);
        process_expense(p, actual, 3);

        actual = get_dynamic_price(g_prices.utilities_base / 2);
        process_expense(p, actual, 4);

        actual = get_dynamic_price(g_prices.transport_base);
        process_expense(p, actual, 5);
    }

    if (p->car.value > 0 && !p->health.in_clinic) {
        ...
        if (repair_chance < 5) {
            ...
            p->health.stress += 10;
            add_event("Поломка автомобиля", 4, "ТРАНСПОРТ");
        }
    }
    ...
    p->health.physical += (rand() % 5) - 2;
    if (p->health.physical > 100) p->health.physical = 100;
    if (p->health.physical < 95) p->health.physical = 95;
    ...
    if (p->health.mental < 30) {
        ...
        if (g_family.shared_budget_pool >= cost) {
            ...
            if (p->health.mental > 95) p->health.mental = 95;
            if (p->health.stress < 0) p->health.stress = 0;
            add_event(p->name + " посетил психолога", 2, "ЗДОРОВЬЕ");
        }
    }
    ...
    for (int i = 0; i < p->investment_count; i++) {
        ...
        if (diff > 0) {
            ...
            add_event(event_msg, 1, "ИНВЕСТИЦИИ");
        }
        else if (diff < 0) {
            g_current_stats.investment_loss += (-diff);
            string event_msg = "Инвестиции -" + to_string(-diff);
            add_event(event_msg, 4, "ИНВЕСТИЦИИ");
        }
    }
    ...
    if (event < 2) {
        ...
        add_event(p->name + " госпитализирован", 5, "ЗДОРОВЬЕ");
    }
    else if (event < 5) {
        RUB loss = 50000;
        ...
        if (accident < 30) {
            ...
            add_event(p->name + " погиб (ДТП)", 5, "ЗДОРОВЬЕ");
        }
        else {
            ...
            add_event(p->name + " ДТП (авария)", 5, "ЗДОРОВЬЕ");
        }
    }
    else if (event < 13) {
        RUB fine = 10000;
        ...
        p->health.stress += 20;
        p->health.mental -= 15;
        add_event(p->name + " депрессия", 4, "ЗДОРОВЬЕ");
    }
    else if (event < 40) {
        RUB lottery = 100000;
    ...
    if (p->health.physical <= 0) {
        p->health.is_alive = false;
        g_state.deaths_count++;
        add_event(p->name + " умер (здоровье)", 5, "ЗДОРОВЬЕ");
    }
    if (g_family.shared_budget_pool < -500000) {
        p->health.is_alive = false;
        p->health.died_from_debt = true;
        g_state.deaths_count++;
        add_event(p->name + " умер (долги)", 5, "ЗДОРОВЬЕ");
    }
}
void process_wife_month() {
    Person* p = &g_family.wife;
    if (!p->health.is_alive) return;
    ...
    if (p->career.base_salary > 0 && p->career.can_work && !p->health.in_clinic) {
        ...
        if ((g_current_stats.month == 8) && (g_current_stats.year == 2026) && (p->career.base_salary > 50000)) {
            p->career.base_salary = (RUB)(p->career.base_salary * 1.5);
            add_event(p->name + " получила повышение", 1, "КАРЬЕРА");
        }
        if ((g_current_stats.month == 1) && (g_current_stats.year > 2026)) {
            p->career.base_salary = (RUB)(p->career.base_salary * 1.1);
            add_event(p->name + " индексация зарплаты", 1, "КАРЬЕРА");
    ...
    if (p->health.mental < 30) {
        ...
        if (g_family.shared_budget_pool >= cost) {
            ...
            if (p->health.mental > 95) p->health.mental = 95;
            if (p->health.stress < 0) p->health.stress = 0;
            add_event(p->name + " посетила психолога", 2, "ЗДОРОВЬЕ");
        }
    }
    ...
    if (p->health.in_clinic && p->health.mental > 50) {
        p->health.in_clinic = false;
        add_event(p->name + " выписана из клиники", 1, "ЗДОРОВЬЕ");
    }
    ...
    if (rand() % 100 >= promotion_chance) {
        ...
        if (p->career.experience >= 20 && p->career.level == 1) {
            ...
            p->career.base_salary = (RUB)(p->career.base_salary * 1.3);
            p->career.position = "Ведущий специалист";
            add_event(p->name + " повышена", 1, "КАРЬЕРА");
        }
        else if (p->career.experience >= 50 && p->career.level == 2) {
            p->career.level = 3;
            p->career.base_salary = (RUB)(p->career.base_salary * 1.5);
    ...
    if (event < 2) {
        ...
        split_expense_proportional(hospital, &g_family.husband, &g_family.wife);
        g_current_stats.medical_expenses += hospital;
        add_event(p->name + " госпитализирована", 5, "ЗДОРОВЬЕ");
    }
    else if (event < 5) {
        RUB loss = 50000;
        split_expense_proportional(loss, &g_family.husband, &g_family.wife);
        ...
        if (accident < 30) {
            p->health.is_alive = false;
            g_state.deaths_count++;
            add_event(p->name + " погибла (ДТП)", 5, "ЗДОРОВЬЕ");
        }
        else {
            p->health.physical -= 40;
            RUB repair = 200000;
    ...
    if (p->health.physical <= 0) {
        p->health.is_alive = false;
        g_state.deaths_count++;
        add_event(p->name + " умерла (здоровье)", 5, "ЗДОРОВЬЕ");
    }
    if (g_family.shared_budget_pool < -500000) {
        p->health.is_alive = false;
        p->health.died_from_debt = true;
        g_state.deaths_count++;
        add_event(p->name + " умерла (долги)", 5, "ЗДОРОВЬЕ");
    }
}
void process_daughter_month() {
    Person* p = &g_family.daughter;
    if (!p->health.is_alive) return;

    if (!p->health.in_clinic) {
        RUB base = g_prices.food_base / 3;
        RUB actual = get_dynamic_price(base);
        process_expense(p, actual, 1);
    ...
    if (p->health.mental < 0) p->health.mental = 0;
    if (p->health.mental > 85) p->health.mental = 85;
    p->health.mental += (rand() % 8) - 4;
    if (p->health.mental > 85) p->health.mental = 85;
    if (p->health.mental < 50) p->health.mental = 50;
    ...
    if (p->health.mental >= 50) {
    }
    else {
        int shopping_chance = rand() % 100;
        if (shopping_chance < 40) {
            RUB shopping = 5000;
            if (g_family.shared_budget_pool >= shopping) {
                ...
                add_event("Дочь ходит по магазинам", 2, "СЕМЬЯ");
            }
        }
    }

    int steal_chance = rand() % 100;
    if (p->account.balance < 0 && steal_chance < 10) {
    ...
    if (!g_daughter_edu.exams_passed) {
        if (g_current_stats.month == 5 && g_current_stats.year == 2026) {
            int base_score = 50 + (p->health.mental / 5);
    ...
    if (g_daughter_edu.exams_passed && !g_daughter_edu.enrolled) {
        if (g_current_stats.month == 6 && g_current_stats.year == 2026) {
            ...
            if (g_daughter_edu.exam_score >= 80 && g_family.shared_budget_pool >= final_cost) {
                ...
                g_daughter_edu.education_cost = final_cost;
                g_daughter_edu.years_remaining = 4;
                g_daughter_edu.institution_name = "ВУЗ";
                g_daughter_edu.status = "СТУДЕНТ";
                add_event("Поступление в ВУЗ", 1, "ОБРАЗОВАНИЕ");
            }
                ...
                add_event("Не смогла поступить", 3, "ОБРАЗОВАНИЕ");
            }
            ...
            if (g_daughter_edu.education_level >= 2) {
                p->career.can_work = false;
            }
            else {
                p->career.can_work = true;
                p->career.base_salary = 25000;
                init_career(&p->career, 1, "Рабочий", 25000);
            }
        }
    }

    if (g_daughter_edu.enrolled && g_daughter_edu.years_remaining > 0) {
        if (g_current_stats.month == 6) {
            g_daughter_edu.years_remaining--;
            add_event("Год обучения прошел", 0, "ОБРАЗОВАНИЕ");
            if (g_daughter_edu.years_remaining <= 0) {
                g_daughter_edu.enrolled = false;
                p->career.can_work = true;
                if (g_daughter_edu.education_level == 3) {
                    p->career.base_salary = 150000;
                    init_career(&p->career, 2, "Специалист", 150000);
                    add_event("Окончила ВУЗ", 1, "ОБРАЗОВАНИЕ");
                }
                else if (g_daughter_edu.education_level == 2) {
                    p->career.base_salary = 80000;
                    init_career(&p->career, 1, "Рабочий", 80000);
                    add_event("Окончила колледж", 1, "ОБРАЗОВАНИЕ");
                }
            }
        }
    }
    ...
    else if (event < 20 && p->career.can_work && p->career.base_salary > 0) {
        RUB bonus = p->career.base_salary / 4;
        process_income(p, bonus);
        g_current_stats.career_bonus += bonus;
        add_event(p->name + " премия", 1, "КАРЬЕРА");
...
void process_dog_month() {
    ...
    if (pet->health <= 0 || death_chance < 3) {
        pet->is_alive = false;
    ...
    if (pet->health < 50 && !pet->is_ill) {
        RUB vet = pet->vet_cost;
        split_expense_proportional(vet, &g_family.husband, &g_family.wife);
        g_current_stats.medical_expenses += vet;
        pet->health += 30;
...
void process_cat_month() {
    Pet* pet = &g_family.cat;
    if (!pet->is_alive) return;
    ...
    int illness_chance = rand() % 100;
    if (illness_chance < 6 && !pet->is_ill) {
        pet->is_ill = true;
        pet->illness_severity = rand() % 3 + 1;
        add_event("Кот заболел", 4, "ПИТОМЦЫ");
    }
    ...
    if (pet->is_ill) {
        ...
        split_expense_proportional(vet, &g_family.husband, &g_family.wife);
        g_current_stats.medical_expenses += vet;
        pet->health -= 8 * pet->illness_severity;
        int recovery = rand() % 100;
        if (recovery < 65) {
            pet->is_ill = false;
            pet->illness_severity = 0;
            add_event("Кот выздоровел", 1, "ПИТОМЦЫ");
        }
    }

    int death_chance = rand() % 1500;
    if (pet->health <= 0 || death_chance < 2) {
        pet->is_alive = false;
        add_event("Кот умер", 5, "ПИТОМЦЫ");
    ...
    pet->mental_health += 5;
    if (pet->mental_health > 100) pet->mental_health = 100;
}
void process_utilities_month() {
...
void process_family_events_month() {
    ...
    if (g_family.husband.health.stress > 50 || g_family.wife.health.stress > 50) {
        ...
        if (g_family.shared_budget_pool >= cost) {
            ...
            add_event("Семейный отпуск", 1, "СЕМЬЯ");
        }
    }
    ...
    if (g_family.made_major_purchase) {
    ...
    else if (g_family.shared_budget_pool >= 2000000) {
        ...
        g_family.major_purchase_name = "МАШИНА";
        add_event("Куплена машина", 1, "ПОКУПКИ");
    }

    if (!g_family.cat.is_alive) {
        if (g_family.husband.health.is_alive && g_family.wife.health.is_alive) {
            if (g_family.daughter_good_behavior_points >= 20 && g_family.months_without_stealing >= 3) {
                g_family.cat.is_alive = true;
                add_event("Купили нового кота", 1, "ПИТОМЦЫ");
                g_family.daughter.health.mental += 20;
            ...
            if (stray_chance < 5 && !g_family.cat.is_alive) {
                ...
                add_event("Подобрали бездомного кота", 1, "ПИТОМЦЫ");
                g_family.daughter.health.mental += 15;
            }
        }
    }
    ...
    int major_event = rand() % 100;
    if (major_event < 2) {
        add_event("Экономический кризис", 5, "СОБЫТИЯ");
    ...
    else if (major_event < 8) {
        add_event("Экономический бум", 1, "СОБЫТИЯ");
    ...
    else if (major_event < 10) {
        RUB damage = 500000;
        split_expense_proportional(damage, &g_family.husband, &g_family.wife);
        g_current_stats.unexpected_expenses += damage;
        add_event("Пожар в квартире", 5, "СОБЫТИЯ");
    }
    else if (major_event < 12) {
        RUB theft = 150000;
        split_expense_proportional(theft, &g_family.husband, &g_family.wife);
        g_current_stats.unexpected_expenses += theft;
        add_event("Ограбление квартиры", 5, "СОБЫТИЯ");
    }
}
void sync_personal_budgets() {
    Person* members[] = { &g_family.husband, &g_family.wife, &g_family.daughter };
    const int members_count = 3;
    const RUB PERSONAL_LIMIT = 100000;

    for (int i = 0; i < members_count; i++) {
        Person* p = members[i];
        if (!p->health.is_alive) continue;

        if (p->account.balance > PERSONAL_LIMIT) {
            RUB excess = p->account.balance - PERSONAL_LIMIT;
            p->account.balance = PERSONAL_LIMIT;
            g_family.shared_budget_pool += excess;
            add_event(p->name + " перевёл излишек " + to_string(excess) + " руб. в общий бюджет", 0, "Финансы");
        }

        if (p->account.balance < 0) {
            RUB deficit = -p->account.balance;
            if (g_family.shared_budget_pool >= deficit) {
                g_family.shared_budget_pool -= deficit;
                p->account.balance = 0;
                add_event(p->name + " покрыл долг " + to_string(deficit) + " руб. из общего бюджета", 0, "Финансы");
            }
            else {
                RUB available = g_family.shared_budget_pool;
                g_family.shared_budget_pool = 0;
                p->account.balance += available;
                add_event(p->name + " частично покрыл долг из общего бюджета", 4, "Финансы");
            }
        }
    }
}
void process_financial_month() {
    sync_personal_budgets();
    ...
    if (g_family.shared_budget_pool < 0 || total_debt > 500000) {
        if (g_family.shared_budget_pool >= -500000) {
            ...
            if (g_state.bankruptcy_months == 1) {
                add_event("Тяжёлое финансовое положение семьи", 5, "ФИНАНСЫ");
            }
        }
    }
    else {
    ...
    if (g_family.shared_budget_pool < -50000) {
        if (g_family.husband.account.microloan_debt == 0) {
            RUB loan = 100000;
            take_microloan(&g_family.husband, loan);
            g_family.family_stress += 20;
        }
    ...
    if (g_family.shared_budget_pool <= -500000) {
        ...
        if (g_state.bankruptcy_months >= 2) {
            ...
            add_event("Смерть от долгов", 5, "ЗДОРОВЬЕ");
    ...
    if (!g_family.husband.health.is_alive && !g_family.wife.health.is_alive && g_family.daughter.health.is_alive && !g_state.daughter_in_orphanage) {
        ...
        g_family.daughter.health.is_alive = false;
        g_state.deaths_count++;
        add_event("Дочь в детдоме", 5, "СЕМЬЯ");
        g_state.simulation_ended = true;
    }
}
void init_family() {
    init_person(&g_family.husband, "Денис", 35, 95000, 60000, true);
    init_car(&g_family.husband.car, "BMW X5", 5500000, 20000);
    init_career(&g_family.husband.career, 3, "Старший специалист", 95000);

    init_person(&g_family.wife, "Алиса", 33, 75000, 0, true);
    init_car(&g_family.wife.car, "Toyota Camry", 2400000, 15000);
    init_career(&g_family.wife.career, 2, "Специалист", 75000);
    ...
    init_person(&g_family.daughter, "Ангелина", 16, 0, 0, false);
    g_family.daughter.health.mental = 65 + (rand() % 10);
    init_career(&g_family.daughter.career, 0, "Студент/Безработный", 0);
    g_family.daughter.career.can_work = false;

    init_pet(&g_family.dog, "Джина", "Овчарка");
    init_pet(&g_family.cat, "Сэм", "Британский");
...
void run_simulation() {
    while (!g_state.simulation_ended) {
        ...
        process_husband_month();
        process_wife_month();
        process_daughter_month();
        process_dog_month();
        process_cat_month();
        process_utilities_month();
        process_family_events_month();
        process_financial_month();
...
void save_to_file() {
    ...
    if (outfile.is_open()) {
        outfile << "=== ИТОГИ СИМУЛЯЦИИ ===" << endl;
        outfile << "Текущий бюджет: " << g_family.shared_budget_pool << " руб." << endl;
        ...
        outfile << "Муж: " << (g_family.husband.health.is_alive ? "ЖИВ " : "МЕРТВ ") << endl;
        outfile << "Жена: " << (g_family.wife.health.is_alive ? "ЖИВА " : "МЕРТВА ") << endl;
        ...
        outfile << "Хорошее поведение: " << g_family.daughter_good_behavior_points << " очков" << endl;
        outfile << "Собака: " << (g_family.dog.is_alive ? "ЖИВА " : "МЕРТВА ") << endl;
        if (g_family.cat.is_alive) {
            outfile << "Кот: ЖИВ" << (g_family.cat.is_stray ? " (БРОДЯЧИЙ)" : " ") << endl;
        }
        if (g_family.made_major_purchase) {
            outfile << "Покупка: " << g_family.major_purchase_name << endl;
        }
        outfile << endl << "=== СПИСОК СОБЫТИЙ ===" << endl;
        for (int i = 0; i < g_state.event_count; i++) {
