...
void alice_bank_interest(const int month) {
    ...
    if (alice.vtb.credit_debt > 0) {
        RUB credit_interest = alice.vtb.credit_debt * (alice.vtb.credit_rate / 100 / 12);
        alice.vtb.credit_debt += credit_interest;
        alice.vtb.monthly_interest += credit_interest;
        printf("  Проценты по кредиту: +%lld руб\n", credit_interest);
    }
}

void alice_happiness_update(const int month, const int year) {
    int happiness = 70;
    int stress = 30;

    RUB total_capital = alice.vtb.account_rub + alice.vtb.savings_account +
        alice.investments.total_value + alice.car.value;

    if (total_capital > 5000000) {
        happiness += 25;
        stress -= 20;
        printf("    Финансовая свобода: +25 счастья, -20 стресса\n");
    }
    else if (total_capital > 2000000) {
        happiness += 15;
        stress -= 10;
        printf("    Хорошее финансовое положение: +15 счастья, -10 стресса\n");
    }
    else if (total_capital > 500000) {
        happiness += 5;
        stress -= 5;
        printf("    Стабильное финансовое положение: +5 счастья, -5 стресса\n");
    }
    else if (total_capital < 0) {
        happiness -= 40;
        stress += 50;
        printf("    Долги и кредиты: -40 счастья, +50 стресса\n");
    }
    else if (total_capital < 100000) {
        happiness -= 15;
        stress += 20;
        printf("    Низкий уровень сбережений: -15 счастья, +20 стресса\n");
    }

    if (alice.emergency_fund > 300000) {
        happiness += 10;
        stress -= 15;
        printf("    Надежная финансовая подушка: +10 счастья, -15 стресса\n");
    }
    else if (alice.emergency_fund < 50000) {
        happiness -= 10;
        stress += 15;
        printf("    Отсутствие финансовой подушки: -10 счастья, +15 стресса\n");
    }

    if (alice.career.is_office_working) {
        if (alice.career.office_salary > 100000) {
            happiness += 10;
            printf("    Высокая зарплата: +10 счастья\n");
        }

        if (alice.career.years_experience > 5) {
            happiness += 5;
            printf("    Опыт и профессионализм: +5 счастья\n");
        }
    }

    if (alice.career.subscribers_count > 500000) {
        happiness += 20;
        stress -= 10;
        printf("    Популярный блогер: +20 счастья, -10 стресса\n");
    }
    else if (alice.career.subscribers_count > 100000) {
        happiness += 10;
        printf("    Растущий канал: +10 счастья\n");
    }

    if (alice.career.collaborations_count > 10) {
        happiness += 5;
        printf("    Много коллабораций: +5 счастья\n");
    }

    if (alice.travel.is_traveling) {
        happiness += 15;
        stress -= 20;
        printf("    Отдых в путешествии: +15 счастья, -20 стресса\n");
    }

    if (alice.dog.is_alive) {
        happiness += 15;
        printf("    Наличие собаки: +15 счастья\n");

        if (alice.dog.happiness_level > 80) {
            happiness += 10;
            printf("    Счастливая собака: +10 счастья\n");
        }
        else if (alice.dog.happiness_level < 40) {
            happiness -= 10;
            stress += 10;
            printf("    Грустная собака: -10 счастья, +10 стресса\n");
        }

        if (alice.dog.health_level > 80) {
            happiness += 5;
            printf("    Здоровая собака: +5 счастья\n");
        }
        else if (alice.dog.health_level < 50) {
            happiness -= 15;
            stress += 20;
            printf("    Больная собака: -15 счастья, +20 стресса\n");
        }

        if (alice.dog.is_sick) {
            happiness -= 20;
            stress += 25;
            printf("    Собака болеет: -20 счастья, +25 стресса\n");
        }

        if (alice.dog.is_trained) {
            happiness += 5;
            printf("    Хорошо обученная собака: +5 счастья\n");
        }
    }
    else {
        happiness -= 25;
        stress += 20;
        printf("    Потеря собаки: -25 счастья, +20 стресса\n");
    }

    if (alice.health.is_sick) {
        happiness -= 25;
        stress += 35;
        printf("    Болезнь: -25 счастья, +35 стресса\n");
    }
    else {
        happiness += 10;
        stress -= 10;
        printf("    Хорошее здоровье: +10 счастья, -10 стресса\n");
    }

    if (alice.health.steps_per_day > 10000) {
        happiness += 5;
        printf("    Активный образ жизни: +5 счастья\n");
    }

    if (alice.health.sleep_hours >= 8) {
        happiness += 5;
        printf("    Достаточный сон: +5 счастья\n");
    }
    else if (alice.health.sleep_hours < 6) {
        happiness -= 10;
        stress += 15;
        printf("    Недосып: -10 счастья, +15 стресса\n");
    }

    if (alice.health.stress_level < 30) {
        happiness += 10;
        printf("    Низкий уровень стресса: +10 счастья\n");
    }
    else if (alice.health.stress_level > 70) {
        happiness -= 20;
        stress += 25;
        printf("    Высокий уровень стресса: -20 счастья, +25 стресса\n");
    }

    if (alice.apartment.rent.is_rented) {
        if (alice.apartment.rent.monthly_rent < 30000) {
            happiness += 5;
            printf("    Доступная аренда: +5 счастья\n");
        }
        else if (alice.apartment.rent.monthly_rent > 50000) {
            happiness -= 10;
            stress += 10;
            printf("    Дорогая аренда: -10 счастья, +10 стресса\n");
        }

        if (alice.apartment.rent.has_pets_allowed && alice.dog.is_alive) {
            happiness += 5;
            printf("    Квартира с животными разрешена: +5 счастья\n");
        }

        if (alice.apartment.rent.landlord_rating > 7) {
            happiness += 3;
            printf("    Хороший арендодатель: +3 счастья\n");
        }
    }

    if (alice.car.value > 1000000) {
        happiness += 5;
        printf("    Хорошая машина: +5 счастья\n");
    }

    if (alice.car.accidents_count > 0) {
        happiness -= 5 * alice.car.accidents_count;
        stress += 10 * alice.car.accidents_count;
        printf("    Аварии с машиной: -%d счастья, +%d стресса\n",
            5 * alice.car.accidents_count, 10 * alice.car.accidents_count);
    }

    if (alice.travel.trips_count > 0) {
        happiness += alice.travel.trips_count * 2;
        printf("    Опыт путешествий: +%d счастья\n", alice.travel.trips_count * 2);

        if (alice.travel.best_trip_income > 100000) {
            happiness += 10;
            printf("    Успешное путешествие: +10 счастья\n");
        }

        if (alice.travel.total_likes > 1000000) {
            happiness += 15;
            printf("    Популярный контент: +15 счастья\n");
        }
    }

    if (alice.career.subscribers_count > 0) {
        int subs_bonus = alice.career.subscribers_count / 100000;
        if (subs_bonus > 20) subs_bonus = 20;
        happiness += subs_bonus;
        printf("    Признание аудитории: +%d счастья\n", subs_bonus);
    }

    if (alice.education.total > 0) {
        happiness += 10;
        printf("    Саморазвитие: +10 счастья\n");
    }

    if (alice.hobby.total > 0) {
        happiness += 8;
        stress -= 5;
        printf("    Наличие хобби: +8 счастья, -5 стресса\n");
    }

    if (alice.charity.total > 0) {
        happiness += 10;
        printf("    Благотворительность: +10 счастья\n");
    }

    if (month == 1 || month == 2) {
        happiness -= 5;
        stress += 5;
        printf("    Зимняя хандра: -5 счастья, +5 стресса\n");
    }

    if (month == 4 || month == 5) {
        happiness += 8;
        printf("    Весеннее настроение: +8 счастья\n");
    }

    if (month == 6 || month == 7 || month == 8) {
        happiness += 10;
        stress -= 5;
        printf("    Летний сезон: +10 счастья, -5 стресса\n");
    }

    if (month == 10 || month == 11) {
        happiness -= 3;
        stress += 3;
        printf("    Осенняя усталость: -3 счастья, +3 стресса\n");
    }

    if (month == 12) {
        happiness += 15;
        printf("    Новогоднее настроение: +15 счастья\n");
    }
    if (month == 3 && year == 2026) {
        happiness += 10;
        printf("    Праздничные бонусы: +10 счастья\n");
    }

    RUB investment_profit = alice.investments.total_value - alice.total_invested;
    if (investment_profit > 100000) {
        happiness += 15;
        printf("    Успешные инвестиции: +15 счастья\n");
    }
    else if (investment_profit < -50000) {
        happiness -= 15;
        stress += 20;
        printf("    Убытки от инвестиций: -15 счастья, +20 стресса\n");
    }

    RUB monthly_profit = (alice.salary + alice.career.blogger_income) - alice.food.total -
        alice.apartment.rent.monthly_rent - alice.subscriptions.total;

    if (monthly_profit > 50000) {
        happiness += 10;
        printf("    Положительный денежный поток: +10 счастья\n");
    }
    else if (monthly_profit < 0) {
        happiness -= 15;
        stress += 20;
        printf("    Отрицательный денежный поток: -15 счастья, +20 стресса\n");
    }

    if (alice.is_married) {
        happiness += 20;
        stress -= 10;
        printf("    Семейное положение: +20 счастья, -10 стресса\n");
    }

    if (alice.has_children) {
        happiness += 25;
        stress += 15;
        printf("    Наличие детей: +25 счастья, +15 стресса\n");
    }

    if (alice.age >= 30) {
        happiness -= 5;
        printf("    Возрастные изменения: -5 счастья\n");
    }

    if (alice.food.healthy_food > alice.food.fast_food) {
        happiness += 5;
        printf("    Правильное питание: +5 счастья\n");
    }

    if (alice.food.alcohol > 5000) {
        happiness -= 8;
        stress += 5;
        printf("    Чрезмерное употребление алкоголя: -8 счастья, +5 стресса\n");
    }

    if (alice.health.gym_cost > 0) {
        happiness += 7;
        stress -= 5;
        printf("    Регулярные тренировки: +7 счастья, -5 стресса\n");
    }

    if (alice.subscriptions.total > 5000) {
        happiness += 3;
        printf("    Развлечения и контент: +3 счастья\n");
    }

    if (alice.credit.debt > 0) {
        happiness -= 20;
        stress += 25;
        printf("    Кредитная нагрузка: -20 счастья, +25 стресса\n");
    }

    if (alice.vtb.credit_debt > 0) {
        happiness -= 15;
        stress += 20;
        printf("    Банковский кредит: -15 счастья, +20 стресса\n");
    }

    if (alice.apartment.utility_debt > 0) {
        happiness -= 10;
        stress += 15;
        printf("    Долги по коммуналке: -10 счастья, +15 стресса\n");
    }

    if (alice.car.fine > 0) {
        happiness -= 5;
        stress += 10;
        printf("    Штрафы: -5 счастья, +10 стресса\n");
    }

    if (alice.apartment.renovation_progress > 0 && alice.apartment.renovation_progress < 100) {
        stress += 10;
        printf("    Ремонт в процессе: +10 стресса\n");
    }
    else if (alice.apartment.renovation_progress >= 100) {
        happiness += 15;
        printf("    Завершенный ремонт: +15 счастья\n");
    }

    int random_event = rand() % 100;
    if (random_event < 5) {
        int surprise = 5 + (rand() % 15);
        happiness += surprise;
        printf("    Приятный сюрприз: +%d счастья\n", surprise);
    }
    else if (random_event > 95) {
        int misfortune = 5 + (rand() % 15);
        happiness -= misfortune;
        stress += misfortune;
        printf("    Неприятное событие: -%d счастья, +%d стресса\n", misfortune, misfortune);
    }

    if (month == 4 && alice.tax.is_tax_declared) {
        stress += 15;
        printf("    Налоговый сезон: +15 стресса\n");
    }

    if (alice.vtb.total_interest_earned > 10000) {
        happiness += 5;
        printf("    Пассивный доход: +5 счастья\n");
    }

    if (alice.career.videos_count > 200) {
        happiness += 8;
        printf("    Творческая реализация: +8 счастья\n");
    }

    if (alice.travel.trips_count >= 3 && alice.career.office_salary > 80000) {
        happiness += 12;
        printf("    Хороший баланс работы и отдыха: +12 счастья\n");
    }

    if (alice.charity.environmental > 0) {
        happiness += 7;
        printf("    Забота об экологии: +7 счастья\n");
    }
    ...
    alice.happiness_index = happiness;
    alice.stress_index = stress;

    printf("\n    ========== ИТОГ ==========\n");
    printf("    ИТОГОВОЕ СЧАСТЬЕ: %d / 100\n", happiness);
    printf("    ИТОГОВЫЙ СТРЕСС: %d / 100\n", stress);

    if (happiness >= 80) {
        printf("    Состояние: Алиса на пике счастья!\n");
    }
    else if (happiness >= 60) {
        printf("    Состояние: ХОРОШЕЕ настроение\n");
    }
    else if (happiness >= 40) {
        printf("    Состояние: НОРМАЛЬНОЕ состояние\n");
    }
    else if (happiness >= 20) {
        printf("    Состояние: ПОДАВЛЕННОЕ настроение\n");
    }
    else {
        printf("    Состояние: Требуется помощь!\n");
    }

    if (stress >= 80) {
        printf("    Предупреждение: КРИТИЧЕСКИЙ УРОВЕНЬ СТРЕССА!\n");
    }
    else if (stress >= 60) {
        printf("    Предупреждение: Высокий уровень стресса\n");
    }
}

void alice_print_travel_statistics() {
    printf("\nСТАТИСТИКА ПУТЕШЕСТВИЙ\n");
    printf("Всего путешествий: %d\n", alice.travel.trips_count);
    printf("Всего потрачено: %lld руб\n", alice.travel.total_travel_cost);
    printf("Всего заработано: %lld руб\n", alice.travel.total_travel_income);
    printf("Чистая прибыль от путешествий: %lld руб\n",
        alice.travel.total_travel_income - alice.travel.total_travel_cost);
    printf("\nОбщая статистика:\n");
    printf("  Всего лайков: %lld\n", alice.travel.total_likes);
    printf("  Всего просмотров: %lld\n", alice.travel.total_views);
    printf("  Всего комментариев: %lld\n", alice.travel.total_comments);
    printf("  Лучший доход за путешествие: %lld руб\n", alice.travel.best_trip_income);
    printf("  Самое длительное путешествие: %d дней\n", alice.travel.longest_trip);
    printf("  Непредвиденные расходы за все путешествия: %lld руб\n", alice.travel.total_unexpected_costs);
}

void print_results(const int year, const int month) {
    RUB total_capital = alice.vtb.account_rub + alice.vtb.account_usd * alice.vtb.rate_usd_rub +
        alice.vtb.account_eur * alice.vtb.rate_eur_rub + alice.car.value +
        alice.investments.total_value + alice.vtb.savings_account;

    printf("Инвестиции %d-%02d | Капитал: %lld руб | Счастье: %d\n", year, month, total_capital, alice.happiness_index);
}
...
void simulation() {
    ...
    while (!(year == 2029 && month == 12)) {
        printf("\n================== %d-%02d ==================\n", year, month);

        printf("\n>>> ДОХОДЫ:\n");
        alice_office_salary(month, year);
        alice_blogger_income(month, year);
        alice_savings_distribution();

        printf("\n>>> РАСХОДЫ:\n");
        alice_food_cost(month, year);
        alice_dog_cost(month, year);
        alice_apartment_cost(month, year);
        alice_subscriptions_cost(month);
        alice_clothing_cost(month, year);
        alice_education_cost(month, year);
        alice_health_cost(month, year);
        alice_hobby_cost(month, year);
        alice_charity_cost(month, year);
        alice_utilities_cost(month, year);
        alice_car_cost();

        printf("\n>>> ПУТЕШЕСТВИЕ:\n");
        alice_travel_cost(month, year);

        printf("\n>>> ИНВЕСТИЦИИ:\n");
        alice_investments(month);
        alice_stock_market_update(month);

        printf("\n>>> БАНК И НАЛОГИ:\n");
        alice_bank_interest(month);

        if (month == 12) {
            alice_tax_payment(year);
        }

        printf("\n>>> СЧАСТЬЕ:\n");
        alice_happiness_update(month, year);

        if (alice.vtb.account_rub < 0) {
            printf("\n!!! ОТРИЦАТЕЛЬНЫЙ БАЛАНС! %lld руб !!!\n", alice.vtb.account_rub);
        }

        if (month % 3 == 0) {
            print_results(year, month);
        }
        ...
        if (month > 12) {
            ...
            alice.age++;
            printf("\nНОВЫЙ %d ГОД!\n", year);
            printf("Алисе исполнилось %d лет\n", alice.age);

            Percent inflation = alice_inflation(year);
            printf("Годовая инфляция: %.1f%%\n", inflation);

            alice.career.office_salary += alice.career.office_salary * (inflation / 100);
            printf("Офисная зарплата увеличена: %lld руб\n", alice.career.office_salary);

            if (alice.emergency_fund < 300000) {
                RUB to_emergency = 10000;
                alice.emergency_fund += to_emergency;
                alice.vtb.account_rub -= to_emergency;
            }
        }
    }

    printf("\n\n ИТОГОВЫЙ РЕЗУЛЬТАТ\n");
    RUB final_capital = alice.vtb.account_rub + alice.investments.total_value + alice.car.value;
    printf("Путешествий: %d | Капитал: %lld руб | Счастье: %d\n",
        alice.travel.trips_count, final_capital, alice.happiness_index);
    printf("Доход от блога: %lld руб | Расходы на собаку: %lld руб\n",
        alice.travel.total_travel_income, alice.dog.total_spent);

    alice_print_travel_statistics();
}
...
int main() {
    ...
    printf("СИМУЛЯЦИЯ ЖИЗНИ АЛИСЫ-ВИДЕОБЛОГЕРА\n");
    printf("Период: 2026-2029 гг.\n");
