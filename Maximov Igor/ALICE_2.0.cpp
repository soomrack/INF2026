...
void print_result(){
    printf("\n=== Счета Alice ===\n");
    printf("Личный счет = %lld\n", Alice.bank.account);
    printf("Личный депозит = %lld\n", Alice.bank.deposit);
    printf("Зарплатный счет = %lld\n", Alice.salary_bank.account);
    printf("Зарплатный депозит = %lld\n", Alice.salary_bank.deposit);
    printf("Бизнес счет = %lld\n", Alice.business_bank.account);
    printf("Бизнес депозит = %lld\n", Alice.business_bank.deposit);
    printf("Стоимость машины = %lld\n", Alice.car.value);
    if (Alice.loan.active) {
        printf("Остаток кредита = %lld (проценты %.2f%%)\n", Alice.loan.remaining, Alice.loan.interest_rate);
    } else {
        printf("Кредит: нет\n");
    }
    printf("Общий капитал = %lld\n", 
...
void print_result_peter() {
    printf("\n=== Счета Питера ===\n");
    printf("Банковский счет = %lld\n", Peter.bank.account);
    printf("Банковский депозит = %lld\n", Peter.bank.deposit);
    printf("Стоимость машины = %lld\n", Peter.car.value);
    if (Peter.mortgage.active) {
        printf("Остаток ипотеки = %lld (проценты %.2f%%, осталось месяцев %d)\n",
...
void family_contributions() {
    ...
    if (Alice.salary_bank.account >= alice_contrib) {
        ...
        if (Alice.bank.account >= alice_contrib) {
            ...
            if (Alice.bank.deposit >= from_deposit) {
                ...
                printf("Предупреждение: Нет денег на вклад Алисы в семейный бюджет (необходимо %lld)\n", alice_contrib);
    ...
    if (Peter.bank.account >= peter_contrib) {
        ...
        if (Peter.bank.deposit >= from_deposit) {
            ...
            printf("Предупреждение: Нет денег на вклад Питера в семейный бюджет (необходимо %lld)\n", peter_contrib);
...
void family_mortgage_payment() {
    ...
    if (Family.account >= need) {
        ...
        if (Family.deposit >= from_deposit) {
            ...
            printf("Предупреждение: Нет денег в семейном бюджете для оплаты ипотеки (нужно %lld)\n", need);
...
void family_loan_payment() {
    ...
    if (Family.account >= need) {
        ...
        if (Family.deposit >= from_deposit) {
            ...
            printf("Предупреждение: Нет денег в семейном бюджете для оплаты кредита (нужно %lld)\n", need);
            RUB paid = Family.account + Family.deposit;
...
void alice_fund_personal() {
    ...
    if (Alice.salary_bank.account >= total_expenses) {
        ...
        printf("Предупреждение: Недостаточно денег на зарплатном счете для покрытия личных расходов (нужно %lld, имеется %lld)\n",
...
void alice_food(const int month){
    ...
    if (Alice.bank.account >= need) {
        ...
        if (Alice.bank.deposit >= from_deposit) {
            ...
            printf("Предупреждение: Нет денег на еду (необходимо %lld, имеем %lld на аккаунте, %lld на депозите)\n",
...
void alice_car(const int month){
    ...
    if (Alice.bank.account >= need) {
        ...
        if (Alice.bank.deposit >= from_deposit) {
            ...
            printf("Предупреждение: Нет денег на бензин (необходимо %lld, имеем %lld на аккаунте, %lld на депозите)\n",
                   need, Alice.bank.account, Alice.bank.deposit);
            Alice.bank.deposit = 0;
            Alice.bank.account = 0;
        }
...
void alice_hamster(const int month){
    if (month == 1) {
    ...
    if (Alice.bank.account >= need) {
        ...
        if (Alice.bank.deposit >= from_deposit) {
            ...
            printf("Предупреждение: Нет денег на хомячка (необходимо %lld, имеем %lld на аккаунте, %lld на депозите)\n",
                   need, Alice.bank.account, Alice.bank.deposit);
            Alice.bank.deposit = 0;
            Alice.bank.account = 0;
        }
...
if (month == 1) {
    ...
    if (!married){
        ...
        if (Alice.bank.deposit >= from_deposit) {
            ...
            printf("Предупреждение: Нет денег на аренду (нужно %lld, имеем %lld на аккаунте, %lld на депозите)\n",
                   need, Alice.bank.account, Alice.bank.deposit);
            Alice.bank.deposit = 0;
            Alice.bank.account = 0;
            }
...
void alice_repair(const int month, const int year){
if (month == 2 && year == 2027){
    ...
    if (Alice.bank.deposit >= repair_cost) {
        ...
        printf("Предупреждение: Нет денег для ремонта (необходимо %lld, имеем %lld на аккаунте, %lld на депозите)\n",
...
if (month == 1) {
    ...
    if (Alice.bank.account >= need) {
        ...
        if (Alice.bank.deposit >= from_deposit) {
            ...
            printf("Предупреждение: Нет денег на одежду (необходимо %lld, имеем %lld на аккаунте, %lld на депозите)\n",
                   need, Alice.bank.account, Alice.bank.deposit);
            Alice.bank.deposit = 0;
            Alice.bank.account = 0;
        }
...
if (month == 1) {
    ...
    if (Alice.bank.account >= need) {
        ...
        if (Alice.bank.deposit >= from_deposit) {
            ...
            printf("Предупреждение: Нет денег на развлечения (необходимо %lld, имеем %lld на аккаунте, %lld на депозите)\n",
                   need, Alice.bank.account, Alice.bank.deposit);
            Alice.bank.deposit = 0;
            Alice.bank.account = 0;
        }
...
void alice_education(const int month) {
if (month == 1) {
    ...
    if (Alice.bank.account >= need) {
        ...
        if (Alice.bank.deposit >= from_deposit) {
            ...
            printf("Предупреждение: Нет денег на образование (необходимо %lld, имеем %lld на аккаунте, %lld на депозите)\n",
                   need, Alice.bank.account, Alice.bank.deposit);
            Alice.bank.deposit = 0;
            Alice.bank.account = 0;
        }
...
if (month == 1) {
    ...
    if (Alice.bank.account >= need) {
        ...
        if (Alice.bank.deposit >= from_deposit) {
            ...
            printf("Предупреждение: Нет денег на лечение (необходимо %lld, имеем %lld на аккаунте, %lld на депозите)\n",
                   need, Alice.bank.account, Alice.bank.deposit);
            Alice.bank.deposit = 0;
            Alice.bank.account = 0;
        }
...
void alice_apartment_expenses(const int month) {
if (month == 1) {
    ...
    if (Alice.bank.account >= need) {
        ...
        if (Alice.bank.deposit >= from_deposit) {
            ...
            printf("Предупреждение: Нет денег на ЖКХ (необходимо %lld, имеем %lld на аккаунте, %lld на депозите)\n",
                   need, Alice.bank.account, Alice.bank.deposit);
            Alice.bank.deposit = 0;
            Alice.bank.account = 0;
        }
    ...
    if (net >= 0) {
        ...
        if (Alice.business_bank.account >= loss) {
            ...
            if (Alice.business_bank.deposit >= from_deposit) {
                ...
                printf("Предупреждение: Нет денег на бизнес счете, чтобы покрыть расходы (необходимо %lld, имеем %lld на аккаунте, %lld на депозите)\n",
...
void alice_bar_furniture_repair() {
    ...
    if (Alice.business_bank.account >= need) {
        ...
        if (Alice.business_bank.deposit >= from_deposit) {
            ...
            printf("Предупреждение: Нет денег на ремонт мебели (необходимо %lld, имеем %lld на аккаунте, %lld на депозите)\n",
    ...
    if (Alice.bank.deposit >= needed) {
        ...
        if (Alice.business_bank.account >= opening_cost) {
            ...
            if (Alice.business_bank.deposit >= from_deposit) {
                ...
                printf("Критическая ошибка: Недостаточно средств для покрытия расходов на открытие после перевода!\n");
                Alice.business_bank.deposit = 0;
                Alice.business_bank.account = 0;
            }
        }
        ...
        Alice_bar.is_open = 1;
        printf("\n=== Бар открыт! ===\n");
        printf("Переведено 5,000,000 с личного депозита.\n");
        printf("Потрачено 3,500,000 на открытие. Остаток на бизнес счете: %lld\n", Alice.business_bank.account);
        printf("Установлены ежемесячные закупки продуктов: %lld, ежегодный ремонт мебели: %lld\n",
               Alice_bar.food_supplies, Alice_bar.furniture_repair);
    } else {
        printf("Предупреждение: Недостаточно денег на личном депозите для открытия бара в 2034 (нужно 5,000,000, имеется %lld)\n", 
               Alice.bank.deposit);
    }
}
...
if (month == 1){
    ...
    if (Alice.bank.account >= need) {
        ...
        if (Alice.bank.deposit >= from_deposit) {
            ...
            printf("Предупреждение: Недостаточно денег для страховки (нужно %lld, имеется %lld на счете, %lld на депозите)\n",
                   need, Alice.bank.account, Alice.bank.deposit);
            Alice.bank.deposit = 0;
            Alice.bank.account = 0;
        }
    }
    printf("Страховка оплачена: %lld\n", need);
    }
}
...
void alice_vacation() {
    ...
    if (Alice.bank.account >= need) {
        ...
        if (Alice.bank.deposit >= from_deposit) {
            ...
            printf("Предупреждение: Недостаточно денег для отпуска (нужно %lld, имеется %lld на счете, %lld на депозите)\n",
                   need, Alice.bank.account, Alice.bank.deposit);
            Alice.bank.deposit = 0;
            Alice.bank.account = 0;
        }
    }
    printf("Потрачено на отпуск: %lld\n", need);
}
...
void alice_christmas_bonus() {
    ...
    printf("Рождественская премия: %lld\n", bonus);
}

void alice_new_car(const int month, const int year) {
if (year == 2031 && month == 1) {   
    RUB old_value = Alice.car.value;
    Alice.bank.deposit += old_value;
    printf("Старая машина продана за %lld\n", old_value);
    
    RUB new_car_cost = 3'500'000;
    if (Alice.bank.deposit >= new_car_cost) {
        Alice.bank.deposit -= new_car_cost;
        Alice.car.value = new_car_cost;
        printf("Новая машина куплена за %lld\n", new_car_cost);
    } else {
        printf("Предупреждение: Недостаточно средств на депозите для покупки новой машины (нужно %lld, имеется %lld)\n",
               new_car_cost, Alice.bank.deposit);
        
        Alice.car.value = Alice.bank.deposit;
        Alice.bank.deposit = 0;
        printf("Куплена более дешевая машина за %lld\n", Alice.car.value);
    }
    if (month == 1) {
    Alice.car.value = Alice.car.value * 106 / 100;
    new_car_cost = new_car_cost * 106 / 100;
    } 
    }
}
...
void alice_take_loan() {
    if (!Alice.loan.active) {
        ...
        printf("Взят кредит: %lld под %.2f%% годовых\n", loan_amount, Alice.loan.interest_rate);
    } else {
        printf("Кредит уже активен, невозможно взять еще один.\n");
    }
}
...
void alice_repay_loan() {
    ...
    if (Alice.bank.account >= need) {
        ...
        if (Alice.bank.deposit >= from_deposit) {
            ...
            printf("Предупреждение: Недостаточно денег для платежа по кредиту (нужно %lld, имеется %lld на счете, %lld на депозите)\n",
                   need, Alice.bank.account, Alice.bank.deposit);
    ...
    if (Alice.loan.remaining <= 0) {
        Alice.loan.active = 0;
        Alice.loan.remaining = 0;
        printf("Кредит полностью погашен!\n");
    } else {
        printf("Платеж по кредиту: %lld, остаток: %lld\n", need, Alice.loan.remaining);
    }
}
...
void alice_property_tax() {
    ...
    if (Alice.bank.account >= need) {
        ...
        if (Alice.bank.deposit >= from_deposit) {
            ...
            printf("Предупреждение: Недостаточно денег для налога на имущество (нужно %lld, имеется %lld на счете, %lld на депозите)\n",
                   need, Alice.bank.account, Alice.bank.deposit);
            Alice.bank.deposit = 0;
            Alice.bank.account = 0;
        }
    }
    printf("Налог на имущество уплачен: %lld\n", tax);
}
...
void alice_charity() {
    ...
    if (Alice.bank.account >= need) {
        ...
        if (Alice.bank.deposit >= from_deposit) {
            ...
            printf("Предупреждение: Недостаточно денег для благотворительности (нужно %lld, имеется %lld на счете, %lld на депозите)\n",
                   need, Alice.bank.account, Alice.bank.deposit);
            Alice.bank.deposit = 0;
            Alice.bank.account = 0;
        }
    }
    printf("Пожертвование: %lld\n", need);
}
...
void alice_inheritance() {
    ...
    printf("Получено наследство: %lld\n", inheritance);
}
...
void alice_birthday() {
    ...
    if (Alice.bank.deposit >= withdraw) {
        ...
        printf("Предупреждение: Недостаточно денег на личном депозите на день рождения (нужно 100000, имеется %lld)\n",
               Alice.bank.deposit);
        Alice.bank.deposit = 0;
    }
}
...
void peter_freelance(const int month) {
if (month == 1){
...
void peter_food(const int month){
if (month == 1){
    ...
    if (Peter.bank.account >= need) {
        ...
        if (Peter.bank.deposit >= from_deposit) {
            ...
            printf("Предупреждение: Недостаточно денег на еду (нужно %lld, имеется %lld на счете, %lld на депозите)\n",
...
void peter_car(const int month){
if (month == 1){
    ...
    if (Peter.bank.account >= need) {
        ...
        if (Peter.bank.deposit >= from_deposit) {
            ...
            printf("Предупреждение: Недостаточно денег на бензин (нужно %lld, имеется %lld на счете, %lld на депозите)\n",
                   need, Peter.bank.account, Peter.bank.deposit);
            Peter.bank.deposit = 0;
            Peter.bank.account = 0;
        }
...
void peter_dog(const int month){
if (month == 1){
    ...
    if (Peter.bank.account >= need) {
        ...
        if (Peter.bank.deposit >= from_deposit) {
            ...
            printf("Предупреждение: Недостаточно денег на собаку (нужно %lld, имеется %lld на счете, %lld на депозите)\n",
                   need, Peter.bank.account, Peter.bank.deposit);
            Peter.bank.deposit = 0;
            Peter.bank.account = 0;
        }
...
void peter_clothing(const int month) {
if (month == 1){
    ...
    if (Peter.bank.account >= need) {
        ...
        if (Peter.bank.deposit >= from_deposit) {
            ...
            printf("Предупреждение: Недостаточно денег на одежду (нужно %lld, имеется %lld на счете, %lld на депозите)\n",
                   need, Peter.bank.account, Peter.bank.deposit);
            Peter.bank.deposit = 0;
            Peter.bank.account = 0;
        }
...
void peter_entertainment(const int month) {
if (month == 1){
    ...
    if (Peter.bank.account >= need) {
        ...
        if (Peter.bank.deposit >= from_deposit) {
            ...
            printf("Предупреждение: Недостаточно денег на развлечения (нужно %lld, имеется %lld на счете, %lld на депозите)\n",
                   need, Peter.bank.account, Peter.bank.deposit);
            Peter.bank.deposit = 0;
            Peter.bank.account = 0;
        }
...
void peter_education(const int month) {
if (month == 1){
    ...
    if (Peter.bank.account >= need) {
        ...
        if (Peter.bank.deposit >= from_deposit) {
            ...
            printf("Предупреждение: Недостаточно денег на образование (нужно %lld, имеется %lld на счете, %lld на депозите)\n",
                   need, Peter.bank.account, Peter.bank.deposit);
            Peter.bank.deposit = 0;
            Peter.bank.account = 0;
        }
...
void peter_health(const int month) {
if (month == 1){
    ...
    if (Peter.bank.account >= need) {
        ...
        if (Peter.bank.deposit >= from_deposit) {
            ...
            printf("Предупреждение: Недостаточно денег на здоровье (нужно %lld, имеется %lld на счете, %lld на депозите)\n",
                   need, Peter.bank.account, Peter.bank.deposit);
            Peter.bank.deposit = 0;
            Peter.bank.account = 0;
        }
...
void peter_apartment_expenses(const int month) {
if (month == 1){
    ...
    if (Peter.bank.account >= need) {
        ...
        if (Peter.bank.deposit >= from_deposit) {
            ...
            printf("Предупреждение: Недостаточно денег на квартплату (нужно %lld, имеется %lld на счете, %lld на депозите)\n",
                   need, Peter.bank.account, Peter.bank.deposit);
            Peter.bank.deposit = 0;
            Peter.bank.account = 0;
        }
...
void peter_mortgage_payment() {
    ...
    if (Peter.bank.account >= need) {
        ...
        if (Peter.bank.deposit >= from_deposit) {
            ...
            printf("Предупреждение: Недостаточно денег для оплаты ипотеки (нужно %lld, имеется %lld на счете, %lld на депозите)\n",
                   need, Peter.bank.account, Peter.bank.deposit);
    ...
    if (Peter.mortgage.remaining <= 0) {
        ...
        printf("Платеж по ипотеке: %lld, остаток: %lld, осталось месяцев: %d\n", need, Peter.mortgage.remaining, Peter.mortgage.months_left);
    }
}


void peter_insurance(const int month) {
if (month == 1){
    if (month == 1) {
    ...
    if (Peter.bank.account >= need) {
        ...
        printf("Страховка Питера оплачена: %lld\n", need);
    }
}
...
void peter_vacation() {
    ...
    printf("Потрачено на отпуск Питера: %lld\n", need);
}
...
void peter_christmas_bonus() {
    ...
    printf("Рождественская премия Питера: %lld\n", bonus);
}
...
void peter_charity() {
    ...
    printf("Пожертвование Питера: %lld\n", need);
}
...
void peter_inheritance() {
    ...
    printf("Питер получил наследство: %lld\n", inheritance);
}
...
void peter_property_tax() {
    ...
    printf("Налог на имущество Питера уплачен: %lld\n", tax);
}
...
void peter_new_car(const int month, const int year) {
if (year == 2032 && month == 1) { 
    RUB old_value = Peter.car.value;
    Peter.bank.deposit += old_value;
    printf("Старая машина Питера продана за %lld\n", old_value);

    RUB new_car_cost = 4'000'000;
    if (Peter.bank.deposit >= new_car_cost) {
        Peter.bank.deposit -= new_car_cost;
        Peter.car.value = new_car_cost;
        printf("Новая машина Питера куплена за %lld\n", new_car_cost);
    } else {
        printf("Предупреждение: Недостаточно средств на депозите для покупки новой машины (нужно %lld, имеется %lld)\n",
...
void simulation(){
    ...
    while (!((month == 3) && (year == 2037))) {
        ...
        if (!married && year == 2032 && month == 1) {
            married = 1;
            printf("\n=== СВАДЬБА! Алиса и Питер поженились. ===\n");
            printf("Создан семейный бюджет. Они будут жить в доме Питера, вместе платить ипотеку.\n");
            printf("Алиса перестает платить аренду. Оба вносят по 100,000 в месяц в семейный бюджет.\n");
        }
        ...
        alice_education(month);
        alice_health(month);
        alice_apartment_expenses(month);
        alice_repair(month, year);
        ...
        peter_freelance(month);
        peter_food(month);
        peter_car(month);
        peter_dog(month);
        peter_clothing(month);
        peter_entertainment(month);
        peter_education(month);
        peter_health(month);
        peter_apartment_expenses(month);
