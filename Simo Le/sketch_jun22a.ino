...
private:
    ...
    bool forcedVentActive;
    unsigned long forcedVentStartTime;
    const unsigned long FORCED_VENT_DURATION = 20000; // 20 секунд
...
public:
    ...
    MultiVent(int pinNum) {
        ...
        forcedVentActive = false;
        forcedVentStartTime = 0;
        pinMode(pin, OUTPUT);
    ...
    // Для запуска принудительной вентиляции
    void startForcedVent() {
        if (!forcedVentActive) { // Защита от повторного запуска
            forcedVentActive = true;
            forcedVentStartTime = millis();
            Serial.println(F("Vent: FORCED START (heater event)"));
        }
    }
    ...
    void power() {
        ...
        // Проверка принудительной вентиляции
        if (forcedVentActive) {
            if (now - forcedVentStartTime >= FORCED_VENT_DURATION) {
                forcedVentActive = false;
                if (activeState) {
                    digitalWrite(pin, LOW);
                    activeState = false;
                    Serial.println(F("Vent: FORCED END - OFF"));
                }
            } else {
                // Принудительная вентиляция имеет наивысший приоритет
                if (!activeState) {
                    digitalWrite(pin, HIGH);
                    activeState = true;
                }
                return; // Выходим, не проверяем другие условия
            }
        }
    ...
    bool isForcedVentActive() const {
        return forcedVentActive;
    }
};
...
private:
    ...
    bool activeState;
    bool prevActiveState; // Для отслеживания изменения состояния
public:
    bool needHeatByTemp;
    bool needHeatByHum;
    ...
    void power() {
        ...
        if (shouldHeat && !prevActiveState) {
            // Обогреватель включается
            linkedVent->startForcedVent();
        } else if (!shouldHeat && prevActiveState) {
