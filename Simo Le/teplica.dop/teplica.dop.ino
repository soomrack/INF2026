...
public:
    ...
    bool needVentByHum;
    bool needVentByTime;
    bool needVentByHeater; // Новый флаг для принудительной вентиляции от обогревателя
    ...
    MultiVent(int pinNum) {
        ...
        needVentByHeater = false; // Инициализация нового флага
        pinMode(pin, OUTPUT);
    ...
    // Метод для принудительного включения вентиляции
...
public:
    ...
    // Метод для получения состояния (добавлен для отслеживания изменений)
    bool getState() const {
        return activeState;
    }
};
